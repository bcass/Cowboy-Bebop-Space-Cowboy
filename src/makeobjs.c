...
// Vomit Stuff
void make_vomit( CHAR_DATA *ch )
{
        OBJ_DATA *obj;

        obj             = create_object( get_obj_index( OBJ_VNUM_VOMIT ), 0 );
        obj->timer      = number_range( 2, 4 );
        obj->value[1]   = number_range( 3, UMIN(5, ch->top_level) );
        obj_to_room( obj, ch->in_room );
}
// Empty rounds on the floor!
void make_shell( CHAR_DATA *ch )
{
        OBJ_DATA *obj;
	OBJ_DATA *wield;

	// To prevent crashes - Gatz
	wield = get_eq_char( ch, WEAR_WIELD );
    	if ( wield != NULL )
	{
		if( wield->value[3] != WEAPON_SWORD && wield->value[3] != WEAPON_KNIFE)
		{
			if( wield->value[3] == WEAPON_RIFLE)
	        		obj = create_object( get_obj_index( OBJ_VNUM_RIFLE_SHELL ), 0);
			else
				obj = create_object( get_obj_index( OBJ_VNUM_BULLET_SHELL ), 0);
  		      	obj->timer      = number_range( 2, 4 );
        		obj->value[1]   = number_range( 3, UMIN(5, ch->top_level) );
        		obj_to_room( obj, ch->in_room );
		}
...
void make_clip( CHAR_DATA *ch )
{
        OBJ_DATA *obj;

        obj             = create_object( get_obj_index( OBJ_VNUM_EMPTY_CLIP ), 0 );
        obj->timer      = number_range( 2, 4 );
        obj->value[1]   = number_range( 3, UMIN(5, ch->top_level) );
        obj_to_room( obj, ch->in_room );
}
 ...
 * make some coinage
 */
OBJ_DATA *create_money( long long int amount )
{
    char buf[MAX_STRING_LENGTH];
    OBJ_DATA *obj;
    ...
    if ( amount <= 0 )
    {
	bug( "Create_money: zero or negative money %lld.", amount );
	amount = 1;
    }
	...
	obj->value[0]	 = (int) (amount % 2000000000LL);
	obj->value[1]	 = (int) (amount / 2000000000LL);
    }
