...
*/
    ...
    sprintf( log_buf, "Cowboy Bebop: Space Cowboy ready on port %d.", port );
    log_string( log_buf );
    game_loop( );
 /*   close( control  );
    close( control2 );
...
/*  time_t	last_check = 0;  */
 ...
 /* signal( SIGSEGV, SegVio ); */
	...
	{
	    struct timeval now_time;
	    long long int secDelta;
	    long long int usecDelta;
...
//This is the parsing function to check for "clearbuff" - Locke
...
/*	extern int lang_array[];
	...
	// This is to prevent characters from logging in as BOB or GoErgE - Gatz
		...
		close_socket( d, FALSE );
              ...
              sprintf( buf, "\n\rChoosing a name is one of the most important parts of this game...\n\r"
              			"Make sure to pick a name appropriate to the character you are going\n\r"
               			"to role play, and be sure that it suits our Cowboy Bebop theme.\n\r"
	...
	else
	{
	    ...
	    SET_BIT(ch->act,PLR_ANSI);
            ch_printf( ch, "\n\r&rName not in player database, creating new character.\n\r\n\r" );
            sprintf( buf, "Is %s your desired name?", argument);
	    write_to_buffer( d, buf, 0 );
	    send_to_char(" Remember, names from Cowboy Bebop are not authed.&R (&rY&G/&rN&R)&r?&w ", ch);
         //   write_to_buffer( d, buf, 0 );
	...
	{
	case 'y': case 'Y':
		// Changed sprintf to ch_printf
	    ch_printf( ch, "\n\r&gMake sure to use a password that won't be easily guessed by someone else."
	...
	case 'n': case 'N':
		// write_to_buffer changed to ch_printf
	    ch_printf( ch, "&GOk, what IS it, then? ", 0 );
	    /* clear descriptor pointer to get rid of bug message in log */
	    d->character->desc = NULL;
	    free_char( d->character );
	    d->character = NULL;
	...
	default:
		// write_to_buffer to ch_printf
	    ch_printf( ch, "&GPlease type Yes or No. " );
	    break;
	}
	break;
...
//	SET_BIT(ch->act,PLR_ANSI);
	...
	ch_printf(ch, "\n\r&RWelcome to Cowboy Bebop: Space Cowboy!"
		    " \n\n&G This MUD combines PK and RP in a unique blend"
		    " of action! This allows players\n to enjoy the"
		    " easy life being a merchant or take a fast"
		    " paced track of a bounty\n hunter! You'll notice"
		    " we don't have gimmicky setups for character"
	...
	//write_to_buffer( d, buf, 0 );
	//write_to_buffer( d, echo_on_str, 0 );
	ch_printf( ch, "\n\r&rWhat is your sex &R(&rM&G/&rF&R)&r?&w " );
...
/*(Draco)i wonder which lame and asshoul added here something like that:
	/*
	write_to_buffer( d, buf, 0 );
	CON_GET_NEW_RACE = 'h';
	break;
...
*/
		ch->race = 0;
     /* Hair Color is teh Suck
        write_to_buffer( d, "\n\rYou may choose from the following hair colors.\n\r\n\rPlease put your selection in quotations:  IE. 'charcoal grey'\n\r", 0 );
        buf[0] = '\0';
        col = 0;
        for ( iHair = 0; iHair <= 32; iHair++)
	{
   ...
   case CON_GET_HAIR:
       ...
       {
       	   do_help(ch, argument);
       	   write_to_buffer( d, "Please choose a hair color: ", 0);
       	   return;
       	}

        for ( iHair = 0; iHair < 32; iHair++ )
	{
	    if ( toupper(arg[0]) == toupper(hair_list[iHair][0])
	    &&   !str_prefix( arg, hair_list[iHair] ) )
	    {
    ...
    if ( iHair == 33
    || str_cmp(arg, hair_list[iHair]) || hair_list[iHair][0] == '\0')
	{
	    write_to_buffer( d, "That's not a hair color.\n\rWhat IS it going to be? ", 0 );
	    return;
	}

            write_to_buffer( d, "\n\rYou may choose from the following eye colors.\n\r\n\rPlease put your selection in quotations:  IE. 'charcoal grey'\n\r", 0 );
        buf[0] = '\0';
        col = 0;
        for ( iEye = 0; iEye <= 19; iEye++)
	{
   ...
   case CON_GET_EYE:
       ...
       {
       	   do_help(ch, argument);
       	   write_to_buffer( d, "Please choose an eye color: ", 0);
       	   return;
       	}

        for ( iEye = 0; iEye < 19; iEye++ )
	{
	    if ( toupper(arg[0]) == toupper(eye_list[iEye][0])
	    &&   !str_prefix( arg, eye_list[iEye] ) )
	    {
		ch->pcdata->eye = iEye;
		break;
	    }
	}
    if ( iEye == 20 ||  str_cmp(arg, eye_list[iEye]) || eye_list[iEye][0] == '\0')
	{
	    write_to_buffer( d, "That's not an eye color.\n\rWhat IS it going to be? ", 0 );
	    return;
	}
   */
	/* Fixed the class choice too... -Spike */
...
//Enter RP Age, Height and Weight - Gatz

	// time to switch over to ch_printf's
	ch_printf( ch, "(Note: Age doesn't affect gameplay, and will remain constant.)\r\n");
	ch_printf( ch,"\n\r&rPlease enter your age &R(&r14&G-&r60&R)&r: ");
...
case CON_GET_NEW_HEIGHT:
	...
	if( x < 100 || x > 200)
	{
		write_to_buffer( d, "\n\rSorry, that isn't in the height range.", 0);
		write_to_buffer( d, "\n\rRange is 100 to 200 cm, enter new height: ", 0);
		return;
	}
	ch->pcheight = x;
...
case CON_GET_NEW_WEIGHT:
    ...
    case CON_ROLL_STATS:
            // New Roll - Gatz
	    ch->perm_str = number_range(8, 12);
	    max -= ch->perm_str;
	    ch->perm_int = number_range(9, 14);
	    max -= ch->perm_int;
	    ch->perm_wis = number_range(8, 13);
	    max -= ch->perm_wis;
	    ch->perm_dex = number_range(8, 13);
	    max -= ch->perm_dex;
	    ch->perm_con = number_range(8, 13);
	    max -= ch->perm_con;
	    if( max > 15)
		max = 15;
	    ch->perm_cha = max;
	...
	ch_printf( ch, "\n\r&rSTR&R:&G %d  &rINT&R:&G  %d &rWIS&R:&G %d&r  DEX&R:&G %d  &rCON&R:&G %d  &rCHA&R:&G %d\n\r" ,
	    ch->perm_str, ch->perm_int, ch->perm_wis,
	    ch->perm_dex, ch->perm_con, ch->perm_cha) ;

//        write_to_buffer( d, buf, 0 );
        ch_printf( ch, "\n\r&rAre these stats OK?  " );
	...
	*/
	    max = 75;
            ...
            ch->perm_str = number_range(8, 12);
            max -= ch->perm_str;
            ch->perm_int = number_range(8, 12);
            max -= ch->perm_int;
            ch->perm_wis = number_range(8, 12);
            max -= ch->perm_wis;
            ch->perm_dex = number_range(8, 12);
            max -= ch->perm_dex;
            ch->perm_con = number_range(8, 12);
            max -= ch->perm_con;
            if( max > 15)
                max = 15;
            ch->perm_cha = max;
...
*/
    ...
    case CON_READ_MOTD:
	write_to_buffer( d, "\n\rWelcome to Space Cowboy.\n\r\n\r", 0 );
	add_char( ch );
	d->connected	= CON_PLAYING;
	/*do_focus pointer cleared. Jet/Kristen 08-14-03*/
	// Major bug fix - Gatz
       	ch->focus = NULL;
	...
	if ( ch->top_level == 0 )
	{
	    ...
	    ch->perm_lck = number_range(6, 18);
            ch->adren = number_range(-2000, 20);
	    ch->affected_by	  = race_table[ch->race].affected;
	    ch->perm_lck	 += race_table[ch->race].lck_plus;
	    ch->adren	 += race_table[ch->race].frc_plus;

            if ( ch->race == RACE_DUINUOGWUIN || ch->main_ability == FORCE_ABILITY )
               ch->adren = URANGE( 1 , ch->adren , 20 );
	    else
	       ch->adren = URANGE( 0 , ch->adren , 20 );
	    ...
	    */
	    ...
	    ch->gold    = 8000;
	    if ( ch->adren > 0 )
         	 ch->max_mana = 100 + 100*ch->adren;
	    else
	         ch->max_mana = 0;
	    ch->mana	= ch->max_mana;
	    sprintf( buf, " the Space Wanderer",
		race_table[ch->race].race_name );
	    do_title( ch, buf );
	...
	{
	    OBJ_INDEX_DATA *obj_ind = get_obj_index( 1414 );
	    if ( obj_ind != NULL )
	    {
	...
	{
	    OBJ_INDEX_DATA *obj_ind = get_obj_index( 1412 );
	    if ( obj_ind != NULL )
	    {
	    obj = create_object( obj_ind, 0);
	    obj_to_char( obj, ch );
	...
	{
	    OBJ_INDEX_DATA *obj_ind = get_obj_index( 1410 );
	    if ( obj_ind != NULL )
	    {
	    obj = create_object( obj_ind, 0);
	    obj_to_char( obj, ch );
	...
	{
	    OBJ_INDEX_DATA *obj_ind = get_obj_index( 1413 );
	    if ( obj_ind != NULL )
	    {
	    obj = create_object( obj_ind, 0);
	    obj_to_char( obj, ch );
    ...
    do_look( ch, "auto" );
    do_global_boards( ch, "" );
    do_fixac(ch,ch->name); /* Ooops, I broke a lot of people's ac, here's a little fix - Funf */
	// This goes to the war code - Gatz
    CLAN_DATA *clan;
	...
	if(clan)
	{
		...
		{
			send_to_char("&W-------------------------------------------\n\r", ch);
			ch_printf(ch, "&RYou are currently at war with: %s%s%s.",
...
bool check_multi( DESCRIPTOR_DATA *d , char *name )
{
        ...
        && !str_cmp(dold->host , d->host ) )
	{
	        ...
	        if ( get_trust( d->character ) >= LEVEL_INFINITE
		|| get_trust( dold->original ? dold->original : dold->character ) >= LEVEL_INFINITE )
		    return FALSE;
...
void do_name( CHAR_DATA *ch, char *argument )
{
  ...
  argument[0] = UPPER(argument[0]);
  // Below prevents problems with names being messed up with caps - Gatz
  argument = capitalize(argument);
  ...
  * "the Space Wander" in it. -Gatz
  ...
  strcat( ch->pcdata->title , " the Space Wanderer" );
  */
...
/*  make_color_sequence(NULL, NULL, NULL);*/
  for ( ; *prompt; prompt++ )
  {
	...
	*pbuf = '\0';
	...
	break;
      case 'g':
	// Changed to make it pretty with commas
	//	stat = ch->gold;
	sprintf(pbuf, "%s", num_punct(ch->gold));
	break;
      case 'G':
	sprintf(pbuf, "%s", (IS_NPC(ch) ? "0" : num_punct(ch->pcdata->bank)));
	break;
      case 'r':
	if ( IS_IMMORTAL(och) )
  ...
  if (!fp)
   ...
   if(!str_cmp(argument, "poscrash"))
   {
	...
	switch(message)
	{
		...
		case 1:
		do_echo(ch, "^p ^x &CPunch: Aye Dios Mio! Possible crash!&w ^p ^x");
		break;
		case 2:
		do_echo(ch, "^p ^x &PJudy: Eeps, possible crash! I hope I don't break a nail!&w ^p ^x");
		break;
		case 3:
		do_echo(ch, "^p ^x &CPunch: Hold it Amigos, a crash may happen!&w ^p ^x");
		break;
		case 4:
		do_echo(ch, "^p ^x &PJudy: Ahhhhhh, my hair, possible crash!!!!!&w ^p ^x");
		break;
	}
	return;
   }
