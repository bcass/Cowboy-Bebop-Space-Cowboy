...
{
    "none",
    "vibro-axe",  "knife",  "sword", "whip", "claw",
    "rifle",  "w7", "bludgeon", "pistol", "w10",
    "heavy weapon", "w12"
};

char *  const   spice_table    [] =
{
    "bloody eye","yellow eye","purple eye","blue eye","green eye","s9"
};
...
char *  const   r_flags2 [] =
{
"gym", "library", "broadcast", "hospital", "light", "computer", "yoga",
"visitors", "r8", "r9", "r10", /* visitors added - Funf */
"r11",
...
"_missile_", "ankle", "back","face","hip","r4","r5","r6",
...
{
"nopkill", "jail", "r2", "r3", "r4", "r5", "r6", "r7", "r8",
...
"corpse", "corpse_pc", "fountain", "pill", "blood", "_bloodstain",
...
"applicator", "computer", "monitor", "case", "harddrive", "motherboard", "shipcomponent"
};
...
"stripsn", "remove", "dig", "full", "thirst", "drunk", "blood", "bleeding"
};
...
"scrying", "frenzy", "shockshield", "r1", "bloody_eye", "possess",
"berserk", "aqua_breath" };
...
{
"npc", "rp", "shovedrag", "autoexits", "autoloot", "autosac", "blank",
...
"wield", "hold", "dual_wield", "ears", "eyes", "missile_wield", "ankle1",
"ankle2", "back", "face", "hip"
};
...
void start_editing( CHAR_DATA *ch, char *data )
{
	...
	// 4096 -> 16384 for larger buffer
	   if ( lines >= 193 || size > 16384 )
	   {
		edit->line[lines][lpos] = '\0';
		break;
	   }
	}
	if ( lpos > 0 && lpos < 78 && lines < 192 )
	{
...
*/
	...
	send_to_char( "  clan vip wanted age weight height rank\n\r",        ch );
	send_to_char( "  strtrain dextrain wistrain inttrain contrain chatrain\n\r",        ch ); /* added by Funf */
	send_to_char( "\n\r",						ch );
	...
	lvalue = is_number( arg3 ) ? atoll( arg3 ) : -1;
	value = (int) lvalue;
   ...
   if ( !str_cmp( arg2, "age" ) )
    {
        if ( IS_NPC(victim) )
        {
            send_to_char( "You can only set age for players!\n\r", ch );
            return;
        }
        ...
        if ( value < 0 )
        {
            send_to_char( "Age range is 14 through 60\n\r", ch );
            return;
        }
        victim->pcage = value;
        send_to_char( "Done.\n\r", ch );
  ...
  if ( !str_cmp( arg2, "weight" ) )
    {
        if ( IS_NPC(victim) )
        {
            send_to_char( "You can only set weight for players!\n\r", ch );
            return;
        }
        if ( !can_mmodify( ch, victim ) )
          return;
        if ( value < 0 )
        {
	    send_to_char( "Number cannot be negitive!\n\r",ch);
  ...
  if ( !str_cmp( arg2, "height" ) )
    {
        if ( IS_NPC(victim) )
        {
            send_to_char( "You can only set height for players!\n\r", ch );
            return;
        }
        if ( !can_mmodify( ch, victim ) )
          return;
        if ( value < 0 )
        {
	    send_to_char( "Number can't be negitive!\n\r",ch);
 ...
 */
	...
	victim->adren = value;
	if ( IS_NPC(victim) && IS_SET( victim->act, ACT_PROTOTYPE ) )
	  victim->pIndexData->adren = value;
	return;
    }

    /* Added whole block of stattrain msetting code - Funf */
    if ( !str_cmp( arg2, "strtrain" ) )
	    {
		if ( !can_mmodify( ch, victim ) )
		  return;
		victim->strtrain = value;
		return;
    }

    if ( !str_cmp( arg2, "dextrain" ) )
	{
		if ( !can_mmodify( ch, victim ) )
		  return;
		victim->dextrain = value;
		return;
    }

    if ( !str_cmp( arg2, "inttrain" ) )
	{
		if ( !can_mmodify( ch, victim ) )
		  return;
		victim->inttrain = value;
		return;
    }

    if ( !str_cmp( arg2, "wistrain" ) )
	{
		if ( !can_mmodify( ch, victim ) )
		  return;
		victim->wistrain = value;
		return;
    }

    if ( !str_cmp( arg2, "contrain" ) )
	{
		if ( !can_mmodify( ch, victim ) )
		  return;
		victim->contrain = value;
		return;
    }

    if ( !str_cmp( arg2, "chatrain" ) )
	{
		if ( !can_mmodify( ch, victim ) )
		  return;
		victim->chatrain = value;
		return;
    }
    ...
    if ( !str_cmp( arg2, "wulongs" ) )
    {
	if ( !can_mmodify( ch, victim ) )
	  return;
	victim->gold = lvalue;
	if ( IS_NPC(victim) && IS_SET( victim->act, ACT_PROTOTYPE ) )
	  victim->pIndexData->gold = lvalue;
	return;
    }
    ...
    if ( !str_cmp( arg2, "hp" ) )
    {
	/* Why not let mobs have more HP? - Gatz
	if ( !can_mmodify( ch, victim ) )
	  return;
	*/
	...
	if ( !argument || argument[0] == '\0' )
	{
	   ...
	   send_to_char( "sentinel, scavenger, aggressive, stayarea, wimpy, practice, immortal,\n\r", ch );
	   send_to_char( "deadly, mountable, guardian, nokill, scholar, noassist, droid, nocorpse,\n\r", ch );
           send_to_char( "pacifist train\n\r", ch );
	   return;
	}
	while ( argument[0] != '\0' )
	{
...
*/
	...
	send_to_char( "  spicetype  grade         charges\n\r",		ch );
...
*/
	...
	if ( !argument || argument[0] == '\0' )
	{
	   ...
	   send_to_char( "Possible Types:\n\r", ch );
           ...
           send_to_char( "Bolt        Chemical   Applicator Computer    Shipcomponent\n\r", ch );
           return;
	}
	...
	if ( !argument || argument[0] == '\0' )
	{
	   send_to_char( "Usage: oset <object> wear <flag> [flag]...\n\r", ch );
           ...
           send_to_char( "wrist  wield    hold    ears    eyes   ankle\n\r", ch );
	   send_to_char( "back   face     hip\n\r", ch );
	   return;
	}
	while ( argument[0] != '\0' )
	{
	...
	case ITEM_WEAPON:
		...
		if ( value < 0 )
		{
		    ...
		    send_to_char( "\n\rChoices:\n\r", ch );
		    send_to_char( "   none, sword, knife, rifle, heavy weapon, pistol, bludgeon\n\r", ch );
		    return;
		}
		tmp = 3;
		break;
	...
	case ITEM_SPICE:
		...
		if ( value < 0 )
		{
		    ...
		    send_to_char( "\n\rChoices:\n\r", ch );
		    send_to_char( "   bloodyeye, carsanum, ryll, andris\n\r", ch );
		    return;
		}
		tmp = 0;
		break;
...
void do_redit( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "  rlist exdistance flags2\n\r",				ch );
	return;
    }
...
*/
    ...
    if ( !str_cmp( arg, "flags2" ) )
    {
        if ( !argument || argument[0] == '\0' )
        {
            send_to_char( "Toggle the second set of room flags.\n\r", ch );
...
void edit_buffer( CHAR_DATA *ch, char *argument )
{
    ...
    /* Added sadiq's format command - Keylord */
...
void do_foldarea( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Usage: foldarea <filename> [remproto] or all\n\r",ch );
	return;
    }

    for ( tarea = first_area; tarea; tarea = tarea->next )
    {
	if ( !str_cmp( tarea->filename, arg ) || !strcmp( arg, "all" ) )
	{
...
void do_astat( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !proto )
    {
	ch_printf( ch, "Max players: %d  IllegalPks: %d  Wulongs Looted: %d\n\r",
			tarea->max_players,
			tarea->illegal_pk,
			tarea->gold_looted );
	if ( tarea->high_economy )
	   ch_printf( ch, "Area economy: %d billion and %d wulongs.\n\r",
			tarea->high_economy,
			tarea->low_economy );
	else
	   ch_printf( ch, "Area economy: %d wulongs.\n\r",
			tarea->low_economy );
        if ( tarea->planet )
	{
	   ch_printf( ch, "Planet: %s.\n\r",
			tarea->planet->name );
	   // Option to generate citizens - Gatz
	   ch_printf( ch, "Generatecit: %d.\r\n",
			tarea->generatecit );
	   if(tarea->generatecit == 1)
	   {
		ch_printf(ch, "Vnums: \r\n");
		ch_printf(ch, "Rare EQ: %d    Low EQ Range: %d   High EQ Range: %d\r\n",
			tarea->rareeq, tarea->loweq, tarea->higheq);
	   }
	}
...
void do_aset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "  author resetmsg resetfreq flags planet generatecit\n\r", ch );
	send_to_char( "  rareeq loweq higheq\r\n", ch);
	return;
    }
    ...
    if( !str_cmp( arg2, "generatecit"))
    {
	if(vnum != 1 && vnum != 0)
	{
		send_to_char("Options are 0 for no, and 1 for yes.\r\n", ch);
		return;
	}
	tarea->generatecit = vnum;
	return;
    }
    if( !str_cmp( arg2, "rareeq"))
    {
	tarea->rareeq = vnum;
        send_to_char( "Done.\n\r", ch );
        return;
