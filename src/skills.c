...
void do_sset( CHAR_DATA *ch, char *argument )
{
	...
	{
	    // This is a small test of a bug fix - Gatz
	    // MAX_LEVEL -> 200
	    skill->min_level = URANGE( 1, atoi( argument ), 200 );
	    send_to_char( "Ok.\n\r", ch );
	    return;
	}
...
void do_search( CHAR_DATA *ch, char *argument )
{
	...
	default:
            ...
            timer = get_do_fun_timerptr( ch, do_search);
            if(timer)
                return;
	    if ( IS_NPC(ch) && IS_AFFECTED( ch, AFF_CHARM ) )
	    {
    ...
    // Search turns up hidden players. - Gatz
        for ( gch = ch->in_room->first_person; gch; gch = gch->next_in_room )
    {
	if(IS_AFFECTED(gch, AFF_HIDE) && !IS_IMMORTAL(gch) && ch != gch)
	{
		peoplehide = TRUE;
		break;
	}
    ...
    if(peoplehide)
    {
	send_to_char("&RYou search turns up some hidden people: \r\n", ch);
	for ( gch = ch->in_room->first_person; gch; gch = gch->next_in_room )
    	{
		if(IS_AFFECTED(gch, AFF_HIDE) && !IS_IMMORTAL(gch) && ch != gch)
			ch_printf( ch, "%s is hiding here!\r\n", gch->name);
	}
    ...
    {
	send_to_char( "You find no items here.\n\r", ch );
...
void do_steal( CHAR_DATA *ch, char *argument )
{
    ...
    if(IS_NPC(victim) && (IS_SET( victim->act, ACT_PACIFIST) || IS_SET(victim->act, ACT_NOSTEAL)))
    {
    ...
    {
	send_to_char("They wouldn't really like that!\r\n", ch);
	return;
    }
    /* Worthless check
    ...
    */
    ...
    // Charm code - Gatz
    if(!IS_NPC(victim) && victim->pcdata->learned[gsn_charm])
    {
	if(get_curr_cha(ch) < get_curr_cha(victim))
	{
		if(number_range(0, (get_curr_lck(victim)/3) + (victim->pcdata->learned[gsn_charm]/25) ) > 3)
		{
			act(AT_RED, "You stop in your tracks because of $N's charm!", ch, NULL, victim, TO_CHAR);
			act(AT_RED, "$n stops in $s tracks and can not steal from you because of your charm!", ch, NULL, victim, TO_VICT);
			act(AT_RED, "$n stops and can not steal from $N because of $S charm!", ch, NULL, victim, TO_NOTVICT);
			learn_from_success(victim, gsn_charm);
			return;
		}
	}
    }

    percent  = number_percent( ) + ( IS_AWAKE(victim) ? 0 : -50 )
	       - (get_curr_dex(ch) - 10) + (get_curr_dex(victim) - 8);

    if ( victim->position == POS_FIGHTING
    ||   percent > ( IS_NPC(ch) ? 10 : ch->pcdata->learned[gsn_steal] ) )
    {
	/*
	 * Failure.
	 */
    ...
    if ( !str_cmp( arg1, "wulongs"  )
    ||   !str_cmp( arg1, "wulong" )
    ||   !str_cmp( arg1, "money"  ) )
    {
	long long int amount;

	amount = (int) (victim->gold / 50 * number_range(1, 5));
	if ( amount <= 0 )
	{
	    send_to_char( "You couldn't get any wulongs.\n\r", ch );
	    learn_from_failure( ch, gsn_steal );
	    return;
	}
	...
	ch->gold     += amount;
	victim->gold -= amount;
	ch_printf( ch, "Aha!  You got %lld wulongs.\n\r", amount );
	...
	if ( IS_NPC( victim ) );
        {
          ...
          ch_printf( ch, "&WYou gain %lld smuggling experience!\n\r", xp );
        }
	return;
    }
    ...
    if ( IS_NPC( victim ) );
    {
      ...
      xp = UMIN( xp , xp_compute( ch, victim ) );
      if ( xp < 0 ) /* Corpses have a negative cost, I guess. Don't let players lose xp. - Funf 2008-05-01 */
		  xp = 0;
      gain_exp( ch, xp, SMUGGLING_ABILITY );
      ch_printf( ch, "&WYou gain %lld smuggling experience!\n\r", xp );
    }
...
void do_backstab( CHAR_DATA *ch, char *argument )
{
	...
	/*  Um, focus...for a stealth attack? - Gatz
	...
	*/
	...
	if( IS_SET( victim->in_room->room_flags, ROOM_PLR_HOME) )
	{
    	...
    	if ( ( obj = get_eq_char( ch, WEAR_WIELD ) ) == NULL || ( obj->value[3] != WEAPON_KNIFE ) )
    	{
...
void do_kick( CHAR_DATA *ch, char *argument )
{
	...
	// Gain exercise!
	if(!IS_NPC(ch) && ch->perm_str < 15)
		ch->strtrain++;
	if(!IS_NPC(victim) && victim->perm_con < 15)
		victim->contrain++;
	return;
}
...
void do_punch( CHAR_DATA *ch, char *argument )
{
	...
	if( !IS_NPC(ch) && ch->perm_str < 15)
		ch->strtrain++;
	if(!IS_NPC(victim) && ch->perm_con < 15)
		victim->contrain++;
	return;
}
...
/* 	Required because of focus	Jet/Kristen 8/11/03 */
	...
	{
		act(AT_PLAIN, "$n fails to disarm $N.", ch, NULL, victim, TO_NOTVICT);
		act(AT_PLAIN, "$n fails to disarm you.", ch, NULL, victim, TO_VICT);
...
/*	ROOM_INDEX_DATA *to_room; */ /* Unused */
    ...
    if ( ( ship = ship_in_room(ch->in_room, arg ) ) != NULL )
    {
   	     ...
   	     if ( IS_NPC(ch) || !ch->pcdata || number_percent( ) > ch->pcdata->learned[gsn_pickshiplock] )
         {
        	...
        	echo_to_room ( AT_RED, ship->in_room , buf );
...
void do_hide( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "You can't in your current position!\n\r", ch);
	return;
    }
...
void do_circle( CHAR_DATA *ch, char *argument )
{
	...
	// Why let Heavy weapon's be able to circle?
	...
	percent = number_percent( ) - (get_curr_dex(ch) - 16) + (get_curr_dex(victim) - 13);
...
/* 	Required because of focus	Jet/Kristen 8/11/03 */
  ...
  /* Hmm.. you get stronger when yer really enraged.. mind over matter
     type thing.. */
  	af.location = APPLY_DAMROLL, APPLY_HITROLL;
  	af.modifier = 5, -5;
