...
long long int advatol (char *s)
{
    long long int number = 0;	/* number to be returned */
...
*/
long long int parsebet (const long long int currentbet, char *s)
{
    /* check to make sure it's not blank */
    if ( s[0] != '\0' )
    {
	/* if first char is a digit, use advatol */
	if ( isdigit(s[0]) )
	    return (advatol(s));
