...
bool	mprog_seval		args( ( char* lhs, char* opr, char* rhs,
                                        CHAR_DATA *mob ) );
bool	mprog_veval		args( ( long long int lhs, char* opr, long long int rhs,
                                        CHAR_DATA *mob ) );
...
bool mprog_veval( long long int lhs, char *opr, long long int rhs, CHAR_DATA *mob )
{
...
*/
	...
	if ( !str_cmp(chck, "waitstate") )
	{
        ...
        // Check players bounty - Gatz
        if ( !str_cmp(chck, "bounty") )
        {
            bounty = get_bounty( chkchar->name);
	    if(!bounty)
		return mprog_veval( 0, opr, atoi(rval), mob);
	    else
                return mprog_veval( bounty->amount, opr, atoi(rval), mob);
        }
	if ( !str_cmp(chck, "hps") )
	{
	...
	if ( !str_cmp( chck, "adren") )
	{
	    return mprog_veval(get_curr_frc(chkchar), opr, atoi(rval), mob);
	}
	if ( !str_cmp(chck, "ismasterglm") )
	{
	    return is_master_glm(chkchar);
	}
    }
    if ( chkobj )
    {
...
*/
  if (current_mpsleep)
  {
      ...
      {
        /*  for (count2 = 0; count2 < DO_ELSE; count2++) */
    	for (count2 = 0; count2 <= DO_ELSE; count2++) /* CRmpsleep - Olc 6May03 */
  ...
  */
  ...
  return;
    ...
    // Added in with the wrapping thing for progs
