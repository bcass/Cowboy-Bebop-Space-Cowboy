...
int get_cost( CHAR_DATA *ch, CHAR_DATA *keeper, OBJ_DATA *obj, bool fBuy )
{
    ...
    if ( ch->gold > MAX_BANK ) /* Used to be: ch->top_level * ch->top_level * 1000 -- Funf */
	richcustomer = TRUE;
    else
	richcustomer = FALSE;
    ...
    if ( fBuy )
    {
	/* cost = (int) obj->cost * ((80. + UMIN(ch->top_level, LEVEL_AVATAR)) / 100.); -- Was broken. Funf */

	 profitmod = 13 - get_curr_cha(ch) + (richcustomer ? 15 : 0)
		  + ((URANGE(5,ch->top_level,LEVEL_AVATAR)-20)/2);
	cost = (int) (obj->cost
	     * (UMAX( (pShop->profit_sell+1.), pShop->profit_buy+profitmod ) ) / 100.);
    }
    else
    {
	OBJ_DATA *obj2;
...
int get_repaircost( CHAR_DATA *keeper, OBJ_DATA *obj )
{
	...
	case ITEM_COMPUTER:
	  if(obj->value[0] >= obj->value[1])
	    ...
	    cost *= ((obj->value[1] - obj->value[0])*2000);
	  break;
      }
    }
...
//	SET_BIT(ch->act, PLR_BOUGHT_PET);
	...
	if( !IS_NPC(ch) && ch->pcdata && ch->pcdata->clan)
	{
		...
		if(planet && planet->governed_by
		  && str_cmp(ch->in_room->area->name, "ISSP HQ")
		  && str_cmp(ch->in_room->area->name, "RDS HQ")
		  && str_cmp(ch->in_room->area->name, "BSS HQ"))
		{
			if(planet->governed_by->war1 || planet->governed_by->war2)
			{
				if(planet->governed_by->war1 == ch->pcdata->clan
				   || planet->governed_by->war2 == ch->pcdata->clan )
				{
					act( AT_TELL, "$n tells you 'I wouldn't be caught dead selling to your"
	          			" clan!'", keeper, NULL, ch, TO_VICT );
            				ch->reply = keeper;
            				return;
				}
			}
	...
	// Planetary Frustration - Gatz
/*	if( !IS_NPC(ch) && planet && planet->frustrated_by && ch->pcdata
	    && ch->pcdata->clan
	    && planet->frustrated_by == ch->pcdata->clan)
	{
		if(planet->frust_level > 20 && planet->frust_level < 50 && clan)
		{
			int xcost = 1;

			xcost = planet->frust_level;
			cost += cost * .1 * (xcost/10 + number_range(1,3));
			ch_printf(ch, "&B%s tells you, 'Since you're a %s you get a price increase!'",
	                        keeper->short_descr, clan->name);
			ch->reply = keeper;
		}
		if(planet->frust_level > 50)
		{
                        act( AT_TELL, "$n tells you 'I wouldn't be caught dead selling to your"
                              " clan!'", keeper, NULL, ch, TO_VICT );
                        ch->reply = keeper;
	...
	if( !IS_NPC(ch) && ch->pcdata->learned[gsn_discount] )
	{
		if(number_range(0,get_curr_cha(ch)/3) > 3)
		{
	                act( AT_TELL, "$n tells you 'I like your charisma! You get a discount!'",
		                keeper, obj, ch, TO_VICT );
	...
	{
 		act( AT_TELL, "$n tells you 'I give discounts to GLM members!'",
                keeper, obj, ch, TO_VICT );
		cost = cost * .90;
	}
	// Spiffy code to allow clans to get a discount from shopkeepers on planets they own
	// - Gatz
  	if( ch->pcdata && ch->pcdata->clan)
  	{
   	...
   	if ( ( planet = ch->in_room->area->planet ) != NULL && clancheck)
   	{
		if(planet->governed_by == clan && planet->pop_support > 10)
		{
			ch_printf(ch, "&B%s tells you, 'Since you're %s, you get a %.0f percent discount!'",
				keeper->short_descr, clan->name, (planet->pop_support/10));
			send_to_char("\n\r",ch);
                	cost -= cost * .01 * (planet->pop_support/10);
		}
   	}
	if(planet && clancheck && planet->governed_by)
	{
		if(planet->governed_by->ally1 || planet->governed_by->ally2)
		{
			if(planet->governed_by->ally1 == clan
			   || planet->governed_by->ally2 == clan)
			{
                        	ch_printf(ch, "&B%s tells you, 'Since you're %s, an ally of %s you get a discount!'",
                        	        keeper->short_descr, clan->name, planet->governed_by->name);
                        	send_to_char("\n\r",ch);
				cost = cost * .95;
			}
		}
	...
	if ( keeper->gold > maxgold )
	{
	    ...
	    act( AT_ACTION, "$n puts some wulongs into a large safe.", keeper, NULL, NULL, TO_ROOM );
	}
...
void do_list( CHAR_DATA *ch, char *argument )
{
	...
	for ( obj = keeper->last_carrying; obj; obj = obj->prev_content )
	{
		...
		// Change to add punct to the number
		if(obj->item_type != ITEM_AMMO && obj->item_type != ITEM_BOLT
		   && obj->item_type != ITEM_WEAPON
		   || (obj->item_type == ITEM_WEAPON && obj->value[3] != 2
			&& obj->value[3] != 6 && obj->value[3] != 9
			&& obj->value[3] != 11 && obj->value[3] != 3))
		{
			ch_printf( ch, "[%5s] {%3d} %s.\n\r",
			    num_punct(cost), oref, capitalize( obj->short_descr ));
		}
		else
		{
			if(obj->item_type == ITEM_AMMO || obj->item_type == ITEM_BOLT)
			{
			     ch_printf( ch, "[%5s] {%3d} %s [%s with %d round].\n\r",
        	                num_punct(cost), oref, capitalize( obj->short_descr ),
				  (obj->item_type == ITEM_AMMO)? "Rifle Ammunition" :
				    (obj->item_type == ITEM_BOLT)? "Pistol Ammunition" : "",
				       obj->value[0] );
			}
			if( obj->item_type == ITEM_WEAPON)
			{
                             ch_printf( ch, "[%5s] {%3d} %s [%s].\n\r",
                                num_punct(cost), oref, capitalize( obj->short_descr ),
				(obj->value[3] == 2)? "Knife" :
				(obj->value[3] == 3)? "Sword" :
				(obj->value[3] == 6)? "Rifle" :
				(obj->value[3] == 9)? "Pistol":
				(obj->value[3] == 11)? "Heavy Weapon": "" );
			}
		}
	       }
	    }
...
void do_sell( CHAR_DATA *ch, char *argument )
{
    ...
    if ( cost > keeper->gold )
    {
	act( AT_TELL, "$n makes a wulong transaction.", keeper, obj, ch, TO_VICT );
        lower_economy( ch->in_room->area, cost-keeper->gold );
    }

    separate_obj( obj );
    act( AT_ACTION, "$n sells $p.", ch, obj, NULL, TO_ROOM );
    sprintf( buf, "You sell $p for %d wulong%s.",
	cost, cost == 1 ? "" : "s" );
...
void do_value( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "$n tells you 'I'll give you %d wulongs for $p.'", cost
);
...
void repair_one_obj( CHAR_DATA *ch, CHAR_DATA *keeper, OBJ_DATA *obj,
                 char *arg, long long int maxgold, char *fixstr, char*fixstr2 )
{
   char buf[MAX_STRING_LENGTH];
   int cost;
   ...
   {
      sprintf( buf,
       "$N tells you, 'It will cost %d wulong%s to %s %s...'", cost,
   ...
   else
   {
      ...
      sprintf( buf, "$N charges you %d wulong%s to %s $p.",
      ...
      if ( keeper->gold > maxgold )
      {
          boost_economy( keeper->in_room->area, keeper->gold - maxgold/2 );
          keeper->gold = maxgold/2;
          act( AT_ACTION, "$n puts some wulongs into a large safe.",
keeper,
		NULL, NULL, TO_ROOM );
      }
...
void appraise_all( CHAR_DATA *ch, CHAR_DATA *keeper, char *fixstr )
{
    ...
    for ( obj = ch->first_carrying; obj != NULL ; obj = obj->next_content )
    {
        ...
        ||   obj->item_type == ITEM_DEVICE ) )
            ...
            {
            sprintf( buf,
            "$N tells you, 'It will cost %d wulong%s to %s %s'",
    ...
    if ( total > 0 )
    {
       send_to_char ("\n\r", ch);
       sprintf( buf,
          "$N tells you, 'It will cost %d wulong%s in total.'",
          total, cost == 1 ? "" : "s" );
       act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
       strcpy( pbuf,
...
void do_appraise( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,
       "$N tells you, 'It will cost %d wulong%s to %s that...'", cost,
       cost == 1 ? "" : "s", fixstr );
    act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
    if ( cost > ch->gold )
      act( AT_TELL, "$N tells you, 'Which I see you can't afford.'", ch,
