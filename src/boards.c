...
void do_note( CHAR_DATA *ch, char *arg_passed, bool IS_MAIL )
{
	...
	else
	{
		...
		if (is_note_to(ch, pnote) || get_trust(ch) > sysdata.read_all_mail)
		{
			...
			{
			    send_to_char("It costs 10 wulongs to read a message.\n\r", ch);
			    return;
			}
	...
	*/
	...
	for ( pnote = board->first_note; pnote; pnote = pnote->next )
	{
 		...
 		if ( take != 0 )
		{
		    ...
		    {
			if ( take == 1 )
			  send_to_char("It costs 50 wulongs to take your mail.\n\r", ch);
			else
			  send_to_char("It costs 50 wulongs to copy your mail.\n\r", ch);
			return;
		    }
		    if ( get_trust(ch) < sysdata.read_mail_free )
		      ch->gold -= 50;
...
void do_bstat( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char("&P+&B--------------&P+&B--------&P+&B------&P+&B------&P+&B------&P+&B------&P+&B-------&P+&B------&P+\n\r"
...
void do_boards( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char("&P+&B--------------&P+&B--------&P+&B------&P+&B------&P+&B------&P+&B------&P+&B-------&P+&B------&P+\n\r"
		...
		"&P+&B--------------&P+&B--------&P+&B------&P+&B------&P+&B------&P+&B------&P+&B-------&P+&B------&P+\n\r",ch);
    for ( board = first_board; board; board = board->next )
		ch_printf( ch, "&P|&W %-12s &P|&W %6d &P|&W %4d &P|&W %4d &P|&W %4d &P|&W %4d &P|&W %5d &P|&W %5d&P|\n\r",
			board->note_file, board->board_obj, board->min_read_level,
			board->min_post_level,board->min_remove_level, board->max_posts,
			board->num_posts, board->type );
	send_to_char("&P+&B------------------------------------------------------------------&P+\n\r",ch);
