...
void do_mpechozone( CHAR_DATA *ch, char *argument ) /* Blod, late 97 */
{
    ...
    CHAR_DATA *vch_next;
    sh_int     color;
  // Was extended bit vector before
    int     actflags;
...
void do_mpsleep( CHAR_DATA *ch, char *argument )
{
    char	arg[ MAX_INPUT_LENGTH ];
	int 	beats;
    ...
    if ( arg[0] == '\0' )
    {
	progbug( "Mpsleep - Bad argument", ch );
	return;
    }
...
void  do_mpgain( CHAR_DATA *ch, char *argument )
{
    ...
    ch_printf( victim, "You gain %lld %s experience.\n\r", exp, ability_name[ability]  );
...
/* lets the mobile kill a player that in a clan which is at war with that clan
   Syntax: mpwarkill <clan> <victim> */

void do_mpwarkill( CHAR_DATA *ch, char *argument )
{
    char arg1[ MAX_INPUT_LENGTH ];
    char arg2[ MAX_INPUT_LENGTH ];
    CHAR_DATA *victim;
    ...
    if (!ch )
    {
        bug( "Nonexistent ch in do_mpwarkill!", 0 );
        return;
    }
/*
    if ( IS_AFFECTED( ch, AFF_CHARM ) )
...
*/
    ...
    {
        progbug( "MpWarKill - no argument", ch );
        return;
    }
    clan = get_clan( arg2 );
    if(!clan)
    {
	progbug( "MpWarKill - Clan is Not real", ch);
	return;
    }

    if ( ( victim = get_char_room_mp( ch, arg1 ) ) == NULL )
    {
        progbug( "mpwarkill - Victim not in room", ch );
        return;
    }
    if(IS_NPC(victim))
	return;
    ...
    if(!vclan)
    {
	progbug("MpWarKill - No Target Clan!", ch);
	return;
    }
    if ( victim == ch )
    {
        progbug( "mpwarkill - Bad victim to attack", ch );
        return;
    }

    if ( IS_AFFECTED( ch, AFF_CHARM ) && ch->master == victim )
    {
        progbug( "mpwarkill - Charmed mob attacking master", ch );
        return;
    }

    if ( ch->position == POS_FIGHTING )
    {
        progbug( "mpwarkill - Already fighting", ch );
        return;
    }
    if(	clan && vclan
	&& (clan->war1 || clan->war2))
    {
	if(clan->war1 == vclan || clan->war2 == vclan)
	{
		multi_hit( ch, victim, TYPE_UNDEFINED );
		return;
	}
...
void do_mposet( CHAR_DATA *ch, char *argument )
{
    ...
    if (!ch )
    {
        bug( "Nonexistent ch in do_mposet!", 0 );
        return;
    }
    ...
    {
		progbug( "mposet - Missing arguments", ch );
		return;
    }
    ...
    if ( argument[0] != '\0' )
    {
		if ( ( victim = get_char_room_mp( ch, argument ) ) != NULL )
		{
			ch = victim; /* Allows for 4th argument. Specifies which char to search for object first - Funf */
		}
	}
    ...
    if ( !obj  )
    {
		...
		{*/
			if ( ( obj = get_obj_here_ch( ch, arg1 ) ) == NULL )
			{
	...
	progbug( "mposet - Invalid arguments (remember, values only)", ch );
    return;
}
