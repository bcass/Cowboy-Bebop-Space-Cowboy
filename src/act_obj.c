...
void get_obj( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container )
{
    ...
    {
	ch->gold += obj->value[0];
	ch->gold += obj->value[1] * 2000000000LL;
	extract_obj( obj );
    }
    else
    {
...
void do_get( CHAR_DATA *ch, char *argument )
{
	...
	if ( ch->in_room && IS_SET(ch->in_room->room_flags, ROOM_PLR_HOME) && !IS_IMMORTAL(ch) )
	{
		if ( !ch->plr_home )
		{
...
/* Editing for Bebop Code done by - Gatz
...
void do_drop( CHAR_DATA *ch, char *argument )
{
...
) )
	{
		...
		case OBJ_VNUM_MONEY_SOME:
		   number += obj->value[0];
		   number += obj->value[1] * 2000000000LL;
		   extract_obj( obj );
		   break;
		}
	    }
...
bool can_dual( CHAR_DATA *ch )
{
   ...
   {
		if ( get_eq_char( ch, WEAR_WIELD )->value[3] == 11 )
   		{
			send_to_char( "You can not dual wield with a heavy weapon!\n\r", ch );
			return FALSE;
   		}
   }
   return TRUE;
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace, sh_int wear_bit )
{
   ...
   if ( check_size )
   {
	...
	/*Ankle Slot Hack for BeBoP - Jet 8-6-3
	 *Just a Copy/Paste/Edit Using ITEM_WRIST Code
	 */
	...
	case ITEM_WEAR_ANKLE:
	   ...
	   case ITEM_WEAR_BACK:
	    ...
	    if ( !can_layer( ch, obj, WEAR_BACK ) )
            ...
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
             if ( !obj->action_desc || obj->action_desc[0]=='\0' )
             {
              act( AT_ACTION, "$n slips $p on $s back.",   ch, obj, NULL, TO_ROOM );
              act( AT_ACTION, "You slip $p on your back.", ch, obj, NULL, TO_CHAR );
             }
             else
             actiondesc( ch, obj, NULL );
            }
	 ...
	 // Rip of hands wear slot - Gatz
    	    case ITEM_WEAR_FACE:
           {
            if ( !can_layer( ch, obj, WEAR_FACE ) )
            {
                send_to_char( "It won't fit overtop of what you're already wearing.\n\r", ch );
                return;
            }
	  ...
	  case ITEM_WEAR_HIP:
           {
            ...
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
             if ( !obj->action_desc || obj->action_desc[0]=='\0' )
             {
              act( AT_ACTION, "$n place $p on $s hip.",   ch, obj, NULL, TO_ROOM );
              act( AT_ACTION, "You place $p on your hip.", ch, obj, NULL, TO_CHAR );
             }
             else
             actiondesc( ch, obj, NULL );
            }
            equip_char( ch, obj, WEAR_HIP );
            oprog_wear_trigger( ch, obj );
            return;
           }
	}
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->pcdata->clan == NULL || ch->pcdata->clan->immortal == NULL )
    {
       sprintf( buf, "$n sacrifices $p to Spike." );
       act( AT_ACTION, buf, ch, obj, NULL, TO_ROOM );
    }
    else
        if(ch->pcdata->clan != NULL && ch->pcdata->clan->immortal != NULL)
	{
	       sprintf( buf, "$n sacrifices $p to %s." );
   	       act( AT_ACTION, buf, ch, obj, ch->pcdata->clan->immortal, TO_ROOM );
	}
    }
...
void do_auction (CHAR_DATA *ch, char *argument)
{
    ...
    if ( ( time_info.hour > 19 || time_info.hour < 6 ) && auction->item == NULL )
    {
        set_char_color ( AT_LBLUE, ch );
    ...
    if (arg1[0] == '\0')
    {
	    ...
	    AFFECT_DATA *paf;
  	    obj = auction->item;
            ...
            /* show item data here */
            if (auction->bet > 0)
                sprintf (buf, "Current bid on this item is %lld wulongs.\n\r",auction->bet);
            else
...
/*	    ch_printf( ch, "Bid: %d\n\r",newbet);	*/
	    ...
	    /* to avoid slow auction, use a bigger amount than 100 if the bet
            ...
            */
            ...
            {
                send_to_char ("You must at least bid 10000 wulongs over the current bid.\n\r",ch);
                return;
            }
	    ...
	    if (newbet > MAX_BANK)
	    {
		send_to_char("You can't bid over 20 billion wulongs.\n\r", ch);
		return;
	    }
		...
		save_char_obj(ch);
            ...
            sprintf (buf,"A bid of %lld wulongs has been received on %s.\n\r",newbet,auction->item->short_descr);
            talk_auction (buf);
            return;
...
/* finally... */
    ...
    if ( atoll(arg2) < 0 )
    {
	send_to_char("You can't auction something for less than 0 wulongs!\n\r", ch);
 	return;
    }
...
*/
	...
	auction->going = 0;
	auction->starting = atoll(arg2);

	if (auction->starting > 0)
	  auction->bet = auction->starting;

	sprintf (buf, "A new item is being auctioned: %s at %lld wulongs.", obj->short_descr, auction->starting);
	talk_auction (buf);
