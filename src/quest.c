...
#define QUEST_ITEM1 10304
#define QUEST_ITEM2 10304
#define QUEST_ITEM3 10304
#define QUEST_ITEM4 10304
#define QUEST_ITEM5 10304


#define QUEST_VALUE1 10000
#define QUEST_VALUE2 5000
#define QUEST_VALUE3 2500
#define QUEST_VALUE4 750
#define QUEST_VALUE5 500
...
#define QUEST_OBJQUEST1 10304
#define QUEST_OBJQUEST2 10304
#define QUEST_OBJQUEST3 10304
#define QUEST_OBJQUEST4 10304
#define QUEST_OBJQUEST5 10304
...
QUEST_VALUE5, obj5->short_descr);
	...
	{
	    send_to_char( "practices nolonger exists!" , ch );
	    return;
        ...
        else if (is_name(arg2, "7"))
	{
		...
		ch->pcdata->quest_curr -= 500;
                ch->gold += 10000000;
                act(AT_MAGIC,"$N gives 10,000,000 wulongs to $n.", ch,
NULL,
			questman, TO_ROOM );
        ...
        if (IS_SET(ch->act, PLR_QUESTOR))
	{
		...
		do_say(questman,buf);
		sprintf(buf,"As a reward, I am giving you %d quest points, and %d wulongs.",pointreward,reward);
		do_say(questman,buf);
    ...
    send_to_char("QUEST commands: POINTS INFO TIME REQUEST COMPLETE LIST BUY FAIL\n\r",ch);
...
void generate_quest(CHAR_DATA *ch, CHAR_DATA *questman)
{
	...
	if ( (vsearch = get_mob_index(mob_vnum) ) != NULL )
	{
		...
		&& vsearch->rShop == NULL
    		...
    		&& !IS_SET(vsearch->act,ACT_IMMORTAL)) break;
		else vsearch = NULL;
	}
    }
