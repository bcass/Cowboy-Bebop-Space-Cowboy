...
void room_explode_1( OBJ_DATA *obj , CHAR_DATA *xch, ROOM_INDEX_DATA *room , int blast )
{
    ...
    for ( rch = room->first_person ; rch ;  rch = rnext )
    {
        ...
        dam = ( number_range ( obj->value[0] , obj->value[1] ) * 5 );
        ...
        if ( !char_died(rch) )
        {
			...
			// Let not spread hate? - Gatz
			if ( !IS_SET( rch->act, ACT_PACIFIST) )
			{
...
int get_exp_worth( CHAR_DATA *ch )
{
    ...
    if ( IS_AFFECTED(ch, AFF_SANCTUARY) )
      exp += exp * 1.5;
	// Slight EXP boost from Frenzy - Gatz
...
sh_int get_curr_str( CHAR_DATA *ch )
{
    ...
    max  = 25 + race_table[ch->race].str_plus;
    max = UMIN(max,25);
...
sh_int get_curr_int( CHAR_DATA *ch )
{
    ...
    max  = 25 + race_table[ch->race].int_plus;
    max = UMIN(max,25);
...
sh_int get_curr_wis( CHAR_DATA *ch )
{
    ...
    max  = 25 + race_table[ch->race].wis_plus;
    max = UMIN(max,25);
...
sh_int get_curr_dex( CHAR_DATA *ch )
{
    ...
    max  = 25 + race_table[ch->race].dex_plus;
...
sh_int get_curr_con( CHAR_DATA *ch )
{
    ...
    max  = 25 + race_table[ch->race].con_plus;
...
sh_int get_curr_cha( CHAR_DATA *ch )
{
    ...
    max  = 25 + race_table[ch->race].cha_plus;
...
sh_int get_curr_lck( CHAR_DATA *ch )
{
    ...
    max  = 25 + race_table[ch->race].lck_plus;
    max = UMIN(max,25);
...
sh_int get_curr_frc( CHAR_DATA *ch )
{
    ...
    return URANGE( 0 , ch->adren + ch->mod_frc, max );
}
...
void char_to_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
	...
	//bug( buf, 0 );
	   ...
	   And I added it back in, bitch. (added a purge though) -Spike  */
 ...
 * This one contains the no death function.
 */
void extract_char( CHAR_DATA *ch, bool fPull, bool capture )
{
    CHAR_DATA *wch;
    OBJ_DATA *obj;
    char buf[MAX_STRING_LENGTH];
...
*/
	...
	extract_obj(obj);*/
    planet = NULL;
    if(ch->in_room->area->planet != NULL)
...
//	    location = get_room_index( ch->pcdata->clan->recall );
	...
	if(!IS_NPC(ch) && capture && !bounty)
	        bounty = get_bounty( ch->name );

	/* Send you to a hospital - Gatz */
	if ( !location  && !IS_NPC(ch))
	{
		if(planet)
		{
			location = get_room_index(planet->hospital);
		}
	}
	if(!IS_NPC(ch) && ch->pcdata && ch->pcdata->bountyrelease > 0)
	{
		location = get_room_index( ROOM_PRISON );
		ch->pcdata->bountyrelease += 2;
	}
	/* Moved to capture_bounty for arrest. --Funf 2008-08-07
	if(!IS_NPC(ch) &&  bounty && !IS_SET(bounty->flags, FAKE_BOUNTY) && bounty->amount > 0 && capture)
	{
		location = get_room_index( ROOM_PRISON );
		ch->pcdata->bountyrelease += (bounty->amount/625000) + 2;
		if( ch->pcdata->bountyrelease > 50 )
			ch->pcdata->bountyrelease = 50;
		if( ch->pcdata->bountyrelease < 5)
			ch->pcdata->bountyrelease = 5;
		ch->pcdata->bountyrelease += (ch->pcdata->bountyrelease * 2);
		if(ch->pcdata->bountyrelease > 100)
			ch->pcdata->bountyrelease = 100;
		ch->pcdata->weaponl += (bounty->amount/625000) + 20;
	...
	if ( ( wch = get_char_room( ch, "healer" ) ) != NULL )
	{
	    ...
	    act( AT_MAGIC, "$n is rushed into the emergency room and bandaged up.", ch, NULL, NULL, TO_ROOM );
...
/*
 * Extract a char from the world.
 * This one is permanent death.
 */
void extract_char2( CHAR_DATA *ch, bool fPull )
{
    CHAR_DATA *wch;
    OBJ_DATA *obj;
    char buf[MAX_STRING_LENGTH];
...
/*
 * Find an obj in the inv of a char anywhere in the room. - Funf
 */

OBJ_DATA *get_obj_here_ch( CHAR_DATA *ch, char *argument )
{
    char arg[MAX_INPUT_LENGTH];
    OBJ_DATA *obj;
    int number, count, vnum;
    ...
    if ( !ch || !ch->in_room )
    {
       bug( "get_obj_wear: null ch or room for ch" );
    }
	...
	for( ch = ch->in_room->first_person ; ch ; ch = ch->next_in_room) /* begin ch search for */
	{
...
bool room_is_private( CHAR_DATA *ch , ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    if ( ch->plr_home && ch->plr_home->vnum == pRoomIndex->vnum ) /* Your apartment isn't private */
    	return FALSE;

	/* Check for War or Bounty - Funf */
    if ( ch->plr_home != pRoomIndex) /* Only check if entering someone elses apartment */
    {
    if ( ch->pcdata && ch->pcdata->clan && IS_SET(pRoomIndex->room_flags, ROOM_PLR_HOME) ) /* Must belong to clan (RBH or ISSP and must be an apartment */
    {
		...
		for ( rch = pRoomIndex->first_person; rch; rch = rch->next_in_room )
		{
			if ( rch->pcdata )
			{
				if ( rch->pcdata->clan && ch->pcdata->clan->war1 )
				{
				...
				if ( rch->pcdata->clan && ch->pcdata->clan->war2 )
				{
...
char *affect_bit_name( int vector )
{
    ...
    if ( vector & AFF_BLOODYEYE     ) strcat( buf, " bloody_eye"    );
    if ( vector & AFF_POSSESS       ) strcat( buf, " possess"       );
...
void add_timer( CHAR_DATA *ch, sh_int type, sh_int count, DO_FUN *fun, int value )
{
    ...
    // removed band aid type != TIMER_DO_FUN
 ...
 */
void boost_economy( AREA_DATA *tarea, long long int gold )
{
 ...
 */
void lower_economy( AREA_DATA *tarea, long long int gold )
{
    while ( gold >= 1000000000 )
    {
 ...
 */
bool economy_has( AREA_DATA *tarea, long long int gold )
{
...
{
   /* Function will remove all & and ^ color codes out of a string */
...
bool is_profane( char *what )
{
    ...
    if (strstr( what, "slut" ))
     return TRUE;
    if (strstr( what, "twat" ) && !strstr( what, "wristwatch" ))
     return TRUE;
...
bool is_master_glm ( CHAR_DATA *ch )
{
	if( ch &&
		ch->pcdata &&
		ch->pcdata->clan &&
		!str_cmp(ch->pcdata->clan->name, "GLM") &&
		ch->pcdata->learned[gsn_makecontainer] == 100 &&
		ch->pcdata->learned[gsn_makecomlink] == 100 &&
		ch->pcdata->learned[gsn_makelight] == 100 &&
		ch->pcdata->learned[gsn_shipmaintenance] == 100 &&
		ch->pcdata->learned[gsn_makeknife] == 100 &&
		ch->pcdata->learned[gsn_makeshield] == 100 &&
		ch->pcdata->learned[gsn_makesword] == 100 &&
		ch->pcdata->learned[gsn_makearmor] == 100 &&
		ch->pcdata->learned[gsn_makejewelry] == 100 &&
		ch->pcdata->learned[gsn_makelandmine] == 100 &&
		ch->pcdata->learned[gsn_makegrenade] == 100 &&
		ch->pcdata->learned[gsn_makegun] == 100 &&
		ch->pcdata->learned[gsn_makerifle] == 100 &&
		ch->pcdata->learned[gsn_makelaptop] == 100)
		return TRUE;
	else
		return FALSE;
}
