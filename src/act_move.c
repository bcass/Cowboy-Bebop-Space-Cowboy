...
char *	const		room_sents[SECT_MAX][25]	=
{
	...
	"Some street people are putting on an interesting display of talent trying to earn some wulongs.",
...
*/
	...
	// Can't move while exercising - Gatz
	/*
	if(IS_SET(ch->pcdata->flags, PCFLAG_EXERCISE))
	{
		send_to_char("&RYou can't move while exercising!\n\r", ch);
		return rNONE;
	}
	*/
	...
	{//spike add these brackets too -Locke
	  ch->move -= move;
	  ch->strtrain += number_range( 1 , (move%10)?move%10:2 );//Passive gain on walk- Locke
	}//--------
    }
...
void do_enter( CHAR_DATA *ch, char *argument )
{
   ...
   if (!IS_NPC(ch) && ch->adren > 1)
   {
	chance = number_range(1,2);
	if(chance == 1)
	{
		send_to_char("&RYour adrenaline rush makes you fumble around with the door!\n\r",ch);
		return;
	}
    }
