...
{
    { "OOC_General",  	"OOC General discussion",            0,     2,     "all", DEF_INCLUDE,21, NULL, FALSE },
    { "OOC_Ideas",	  	"OOC Suggestion for improvement",	 0,     2,     "all", DEF_NORMAL, 60, NULL, FALSE },
    { "OOC_Announce", 	"OOC Announcements from Immortals",  0,     LEVEL_IMMORTAL, "all", DEF_NORMAL, 60, NULL, FALSE },
    { "OOC_Bugs",	  	"OOC Typos, bugs, errors",	         0,     1,     "imm", DEF_NORMAL, 60, NULL, FALSE },
    { "OOC_Personal", 	"OOC Personal messages",	      	 0,     1,     "all", DEF_EXCLUDE,28, NULL, FALSE },
    { "IC_General",  	"IC General discussion",             0,     2,     "all", DEF_INCLUDE,21, NULL, FALSE },
    { "IC_Announce", 	"IC Announcements",   0,     LEVEL_IMMORTAL, "all", DEF_NORMAL, 60, NULL, FALSE },
    { "IC_Personal", 	"IC Personal messages",	      	 	 0,     1,     "all", DEF_EXCLUDE,28, NULL, FALSE }
};
...
long long int last_note_stamp = 0; /* To generate unique timestamps on notes */
...
bool is_note_to (CHAR_DATA *ch, NOTE_DATA *note)
{
    ...
    if (IS_IMMORTAL(ch) && (
                            ...
                            is_full_name ("ap", note->to_list) ||
                            is_full_name ("rbh", note->to_list) ||
                            is_full_name ("glm", note->to_list) ||
                            is_full_name ("rds", note->to_list) ||
                            is_full_name ("bss", note->to_list) ||
                            is_full_name ("issp", note->to_list)))
    ...
    // Spread the gnote love to clannies

    if(!ch->pcdata->clan) return FALSE;
    if(ch->pcdata && ch->pcdata->clan && !str_cmp(ch->pcdata->clan->name, "BSS")
	&& is_full_name("BSS", note->to_list))
		return TRUE;
    if(ch->pcdata && ch->pcdata->clan && !str_cmp(ch->pcdata->clan->name, "ISSP")
	&& is_full_name("ISSP", note->to_list))
		return TRUE;
    if(ch->pcdata && ch->pcdata->clan && !str_cmp(ch->pcdata->clan->name, "RDS")
	&& is_full_name("RDS", note->to_list))
		return TRUE;
    if(!ch->pcdata->clan) return FALSE;
    if(ch->pcdata && ch->pcdata->clan && !str_cmp(ch->pcdata->clan->name, "AP")
        && is_full_name("AP", note->to_list))
                return TRUE;
    if(ch->pcdata && ch->pcdata->clan && !str_cmp(ch->pcdata->clan->name, "RBH")
        && is_full_name("RBH", note->to_list))
                return TRUE;
...
/*
    if((is_full_name("wts", note->to_list)) && (str_cmp(ch->pcdata->clan->name, "wts")))
	 return FALSE;
    if((is_full_name("rds", note->to_list)) && (str_cmp(ch->pcdata->clan->name, "rds")))
	 return FALSE;
    if((is_full_name("issp", note->to_list)) && (str_cmp(ch->pcdata->clan->name, "issp")))
	 return FALSE;

	return TRUE;
 */
	/*
    if(ch->pcdata->clan && (!str_cmp(ch->pcdata->clan->name, "wts") ) &&
	(is_full_name("wts", note->to_list)));
	return TRUE;
...
int unread_notes (CHAR_DATA *ch, GLOBAL_BOARD_DATA *board)
{
    ...
    for (note = board->note_first; note; note = note->next)
        if (is_note_to(ch, note) && ((long long int)last_read < (long long int)note->date_stamp))
            count++;
...
void do_nwrite (CHAR_DATA *ch, char *argument)
{
    ...
    {
		if(!str_cmp(ch->pcdata->board->short_name, "IC_Announce") && ch->pcdata->clan ) //Hard coded special rules for IC announce (2009-02-25 Funf)
		{
			...
			if (!found && str_cmp(ch->pcdata->clan->name, "AP")) // Clan leaders and AP can use IC_Announce
			{
...
void do_nremove (CHAR_DATA *ch, char *argument)
{
    ...
    if (str_cmp(ch->name,p->sender) && (get_trust(ch) < MAX_LEVEL + 5))
    {
...
void do_ncatchup (CHAR_DATA *ch, char *argument)
{
    ...
    else
    {
        ...
        send_to_char_color ("All messages skipped.\n\r",ch);
    }
}
...
void do_global_boards (CHAR_DATA *ch, char *argument)
{
    ...
    if (!argument[0]) /* show boards */
    {
        ...
        count = 1;
        send_to_char_color ("&RNum         Name Unread Description&w\n\r"
                      "&r=== ============ ====== ===========&w\n\r",ch);
        for (i = 0; i < MAX_BOARD; i++)
        {
