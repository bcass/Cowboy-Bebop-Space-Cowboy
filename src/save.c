...
{
    // This prevents loaded up characters from having their apartment's wiped
    if(IS_NPC(ch))
	return;
...
void save_char_obj( CHAR_DATA *ch )
{
    ...
    save_finger( ch ); /*saves PC finger data*/
    saving_char = ch;
...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
    ...
    fprintf( fp, "Weight       %d\n",   ch->pcweight            );
    fprintf( fp, "Height       %d\n",   ch->pcheight            );
    fprintf( fp, "StrTraining  %d\n",   ch->strtrain            );
    fprintf( fp, "WisTraining  %d\n",   ch->wistrain		);
    fprintf( fp, "IntTraining    %d\n",   ch->inttrain		);
    fprintf( fp, "ConTraining  %d\n",   ch->contrain		);
    fprintf( fp, "DexTraining  %d\n",   ch->dextrain		);
    fprintf( fp, "ChaTraining  %d\n",   ch->chatrain		);
    fprintf( fp, "Race         %d\n",	ch->race		);
    ...
    fprintf( fp, "Force        %d %d %d %d\n", ch->adren, ch->mod_frc, ch->mana, ch->max_mana );
    fprintf( fp, "Wulongs      %lld\n",	ch->gold		);
    fprintf( fp, "Bank         %lld\n",	ch->pcdata->bank		);
    {
       int ability;
       for ( ability = 0 ; ability < MAX_ABILITY ; ability++ )
           fprintf( fp, "Ability        %d %d %lld\n",
	...
	if ( IS_IMMORTAL( ch ) || WAS_IMMORTAL( ch ) || ch->pcdata->area )
	{
	...
	if ( ch->pcdata->clan_name && ch->pcdata->clan_name[0] != '\0' )
	  fprintf( fp, "Clan         %s~\n",	ch->pcdata->clan_name	);
	if( ch->pcdata->clantalk1_name && ch->pcdata->clantalk1_name[0] != '\0')
	  fprintf( fp, "Clantalk1    %s~\n",	ch->pcdata->clantalk1_name);
	if( ch->pcdata->clantalk2_name && ch->pcdata->clantalk2_name[0] != '\0')
	  fprintf( fp, "Clantalk2    %s~\n", 	ch->pcdata->clantalk2_name);
	if( ch->pcdata->setclantalk_name && ch->pcdata->setclantalk_name[0] != '\0')
	  fprintf( fp, "Setclantalk  %s~\n",	ch->pcdata->setclantalk_name);
        fprintf( fp, "Flags        %d\n",	ch->pcdata->flags	);
	fprintf( fp, "WeaponL	   %d\n",	ch->pcdata->weaponl	);
	fprintf( fp, "Arrestcount  %d\n",	ch->pcdata->arrestcount );
	fprintf( fp, "Hackcount    %d\n",	ch->pcdata->hackcount);
	fprintf( fp, "MaxArresttime	%d\n",	ch->pcdata->maxarresttime);
	fprintf( fp, "Arresttime	%d\n",	ch->pcdata->arresttime);
        if ( ch->pcdata->bountyrelease > 0 )
            fprintf( fp, "BountyDate       %d\n",
	...
	if ( ch->pcdata->unsilence_date > current_time )
		...
		(int)ch->pcdata->unsilence_date, ch->pcdata->silenceed_by);
	if ( ch->pcdata->pkills )
	  fprintf( fp, "PKills       %d\n",	ch->pcdata->pkills	);
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name, bool preload )
{
    ...
    ch->perm_cha			= 10;
    ch->perm_lck			= 10;
    ch->passive_strtrain = ch->passive_dextrain = ch->passive_contrain = 10;
    ch->passive_inttrain = ch->passive_wistrain = ch->passive_chatrain = 10;
    ch->strtrain = ch->dextrain = ch->contrain = ch->inttrain = ch->wistrain = ch->chatrain = 10;
    ch->pcdata->atrophy = 0;
	...
	ch->pcdata->clan		= NULL;
	ch->pcdata->clantalk1_name	= STRALLOC( "" );
	ch->pcdata->clantalk1		= NULL;
	ch->pcdata->clantalk2_name	= STRALLOC( "" );
	ch->pcdata->clantalk2		= NULL;
	...
	if( !ch->pcdata->clantalk1_name )
	{
	  ch->pcdata->clantalk1_name = STRALLOC( "" );
	  ch->pcdata->clantalk1	     = NULL;
	}

	if( !ch->pcdata->clantalk2_name )
	{
	  ch->pcdata->clantalk2_name = STRALLOC( "" );
	  ch->pcdata->clantalk2	     = NULL;
	}

	if(!ch->pcdata->setclantalk_name)
	{
	  ch->pcdata->setclantalk_name = STRALLOC( "" );
	  ch->pcdata->clantalk2	      = NULL;
	}
...
void fread_char( CHAR_DATA *ch, FILE *fp, bool preload )
{
	...
	case 'A':
	    KEY( "Arresttime",	ch->pcdata->arresttime, fread_number( fp ));
	    KEY( "Arrestcount",	ch->pcdata->arrestcount, fread_number( fp ) );
	    KEY( "Act",		ch->act,		fread_number( fp ) );
	    KEY( "AffectedBy",	ch->affected_by,	fread_number( fp ) );
	    KEY( "Alignment",	ch->alignment,		fread_number( fp ) );
	    KEY( "Armor",	ch->armor,		fread_number( fp ) );
	    KEY( "Atrophy",	ch->pcdata->atrophy,	fread_number( fp ) );
            if ( !str_cmp( word, "AFKMessage" ) )
            {
                ch->pcdata->afkmessage = fread_string( fp );
                if ( isalpha(ch->pcdata->afkmessage[0])
                ||   isdigit(ch->pcdata->afkmessage[0]) )
                {
                    sprintf( buf, " %s", ch->pcdata->afkmessage );
                    if ( ch->pcdata->afkmessage )
                      STRFREE( ch->pcdata->afkmessage );
                    ch->pcdata->afkmessage = STRALLOC( buf );
                }
                fMatch = TRUE;
                break;
            }
	...
	case 'B':
	    KEY( "BountyDate",	ch->pcdata->bountyrelease, fread_number( fp ));
	...
	case 'C':
	    KEY( "ChaTraining", ch->chatrain, fread_number( fp ) );
	    KEY( "ConTraining", ch->contrain, fread_number( fp ) );

            if ( !str_cmp( word, "Clantalk1" ) )
            {
                ch->pcdata->clantalk1_name = fread_string( fp );

                if ( !preload
                &&   ch->pcdata->clantalk1_name[0] != '\0'
                && ( ch->pcdata->clantalk1 = get_clan( ch->pcdata->clantalk1_name )) == NULL )
                {
                  sprintf( buf, "Warning: the organization %s no longer exists, and therefore you no longer\n\rbelong to that organization.\n\r",
                           ch->pcdata->clantalk1_name );
                  send_to_char( buf, ch );
                  STRFREE( ch->pcdata->clantalk1_name );
                  ch->pcdata->clantalk1_name = STRALLOC( "" );
                }
                fMatch = TRUE;
                break;
            ...
            if ( !str_cmp( word, "clantalk2" ) )
            {
                ch->pcdata->clantalk2_name = fread_string( fp );

                if ( !preload
                &&   ch->pcdata->clantalk2_name[0] != '\0'
                && ( ch->pcdata->clantalk2 = get_clan( ch->pcdata->clantalk2_name )) == NULL )
                {
                  sprintf( buf, "Warning: the organization %s no longer exists, and therefore you no longer\n\rbelong to that organization.\n\r",
                           ch->pcdata->clantalk2_name );
                  send_to_char( buf, ch );
                  STRFREE( ch->pcdata->clantalk2_name );
                  ch->pcdata->clantalk2_name = STRALLOC( "" );
                }
                fMatch = TRUE;
                break;
	...
	case 'D':
	    KEY( "DexTraining", ch->dextrain,		fread_number( fp ) );
	    KEY( "Damroll",	ch->damroll,		fread_number( fp ) );
	    KEY( "Deaf",	ch->deaf,		fread_number( fp ) );
	    KEY( "Description",	ch->description,	fread_string( fp ) );
	    if ( !str_cmp( word, "Disguise" ) )
	    {
		ch->pcdata->disguise = fread_string( fp );
		if ( isalpha(ch->pcdata->disguise[0])
		||   isdigit(ch->pcdata->disguise[0]) )
		{
		    sprintf( buf, " %s", ch->pcdata->disguise );
	...
	case 'H':
	    KEY( "Hackcount",	ch->pcdata->hackcount, fread_number( fp ));
	    KEY( "Height",     ch->pcheight,  fread_number( fp ) );
	...
	case 'I':
	    KEY( "IntTraining", ch->inttrain	      , fread_number (fp) );
	...
	case 'M':
	    KEY( "MaxArresttime", ch->pcdata->maxarresttime,	fread_number( fp ));
	...
	case 'P':
	    KEY( "PCAge",       ch->pcage,              fread_number( fp ));
	...
	case 'S':
            KEY( "StrTraining", ch->strtrain,           fread_number( fp ) );
	    KEY( "Sex",		ch->sex,		fread_number( fp ) );
	    KEY( "ShortDescr",	ch->short_descr,	fread_string( fp ) );
	    KEY( "Susceptible",	ch->susceptible,	fread_number( fp ) );
	// New Silence Command
            if ( !str_cmp(word, "Silenced") )
            {
              ch->pcdata->unsilence_date = fread_number(fp);
              ch->pcdata->silenceed_by = fread_string(fp);
              if ( ch->pcdata->unsilence_date < current_time )
              {
		...
		break;
            ...
            if ( !str_cmp( word, "Setclantalk" ) )
            {
                ch->pcdata->setclantalk_name = fread_string( fp );

                if ( !preload
                &&   ch->pcdata->setclantalk_name[0] != '\0'
                && ( ch->pcdata->setclantalk = get_clan( ch->pcdata->setclantalk_name )) == NULL )
                {
                  sprintf( buf, "Warning: the organization %s no longer exists, and therefore you no longer\n\rbelong to that organization.\n\r",
                           ch->pcdata->setclantalk_name );
                  send_to_char( buf, ch );
	...
	case 'W':
	    KEY( "WeaponL",	ch->pcdata->weaponl,	fread_number( fp ) );
            KEY( "WisTraining", ch->wistrain,            fread_number( fp ) );
	    KEY( "Weight",    ch->pcweight,  fread_number( fp ) );
	    KEY("Wulongs",    ch->gold,      fread_long( fp ) );
...
void fread_obj( CHAR_DATA *ch, FILE *fp, sh_int os_type )
{
	...
	case 'N':
	    ...
	    break;
	case 'P':
	    KEY( "PhoneNumber",	obj->phonenumber,	fread_number( fp ) );
	    break;
	case 'R':
	...
	case 'V':
		...
		//if ( file_ver < 3 ) /* This is breaking clan store rooms --Funf 2009-02-04 */
		//   x5=x6=0;
...
void set_alarm( long long int seconds )
{
    alarm( seconds );
}
