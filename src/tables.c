...
DO_FUN *skill_function( char *name )
{
	...
	if ( !str_cmp( name, "do_applylicense"))	return do_applylicense;
	if ( !str_cmp( name, "do_atrophy"))		return do_atrophy;
	if ( !str_cmp( name, "do_attend"))		return do_attend;
	if ( !str_cmp( name, "do_afkmessage"))		return do_afkmessage;
	if ( !str_cmp( name, "do_addradio"))		return do_addradio;
	...
	if ( !str_cmp( name, "do_bigshot" ))		return do_bigshot;
	...
	if ( !str_cmp( name, "do_buygood" ))		return do_buygood;
	if ( !str_cmp( name, "do_background" ))		return do_background;
	if ( !str_cmp( name, "do_bankfreeze" ))		return do_bankfreeze;
	if ( !str_cmp( name, "do_bandage"))		return do_bandage;
	if ( !str_cmp( name, "do_bootup"))		return do_bootup;
	break;
    case 'c':
	...
	if ( !str_cmp( name, "do_commandeer"))		return do_commandeer;
	...
	if ( !str_cmp( name, "do_destroyrestore" ))	return do_destroyrestore;
	if ( !str_cmp( name, "do_destroyslay" ))	return do_destroyslay;
	if ( !str_cmp( name, "do_destroyweapon" ))		return do_destroyweapon;
	if ( !str_cmp( name, "do_detachcomponent" ))		return do_detachcomponent;
	...
	if ( !str_cmp( name, "do_deceive"))		return do_deceive;
	break;
    case 'e':
	...
	if ( !str_cmp( name, "do_empathize" ))		return do_empathize;
	...
	if ( !str_cmp( name, "do_endurance"))		return do_endurance;
	break;
    case 'f':
	if ( !str_cmp( name, "do_fakequit" ))		return do_fakequit;
	if ( !str_cmp( name, "do_fueltransfer" ))	return do_fueltransfer;
	...
	if ( !str_cmp( name, "do_frenzy"))		return do_frenzy;
	break;
    case 'g':
	...
	if ( !str_cmp( name, "do_gemcutting" ))		return do_gemcutting;
        if ( !str_cmp( name, "do_giveplanet" ))         return do_giveplanet;
	...
	if ( !str_cmp( name, "do_group_heal"))		return do_group_heal;
	if ( !str_cmp( name, "do_gainage"))		return do_gainage;
	break;
    case 'h':
//	if ( !str_cmp( name, "do_hack_systems" ))		return do_hack_systems;
	if ( !str_cmp( name, "do_hackbaydoor" ))		return do_hackbaydoor;
	if ( !str_cmp( name, "do_hackcamera"))	return do_hackcamera;
	if ( !str_cmp( name, "do_hackwho" ))		return do_hackwho;
	if ( !str_cmp( name, "do_hackclanaccount"))	return do_hackclanaccount;
	if ( !str_cmp( name, "do_hackbanktransfer"))	return do_hackbanktransfer;
	if ( !str_cmp( name, "do_hackshipdoor"))	return do_hackshipdoor;
	if ( !str_cmp( name, "do_hacklaunch"))		return do_hacklaunch;
	if ( !str_cmp( name, "do_hackbroadcast"))	return do_hackbroadcast;
	if ( !str_cmp( name, "do_hackbounty"))		return do_hackbounty;
	...
	if ( !str_cmp( name, "do_hackbank"))		return do_hackbank;
	...
	if ( !str_cmp( name, "do_installcomponent" ))		return do_installcomponent;
	...
	break;
    case 'l':
	if ( !str_cmp( name, "do_lagout" ))		return do_lagout;
	...
	if ( !str_cmp( name, "do_lift" ))		return do_lift;
	...
	if ( !str_cmp( name, "do_lobby" ))		return do_lobby;
	break;
    case 'm':
	...
	if ( !str_cmp( name, "do_makeknife" ))          return do_makeknife;
	if ( !str_cmp( name, "do_makegun" ))            return do_makegun;
	if ( !str_cmp( name, "do_makerifle"))		return do_makerifle;
	if ( !str_cmp( name, "do_makesword" ))          return do_makesword;
	if ( !str_cmp( name, "do_makerestore" ))	return do_makerestore;
	if ( !str_cmp( name, "do_makeslay" ))		return do_makeslay;
	if ( !str_cmp( name, "do_makespice" ))          return do_makespice;
	if ( !str_cmp( name, "do_makebloodyeye"))       return do_makebloodyeye;
	if ( !str_cmp( name, "do_makeyelloweye"))       return do_makeyelloweye;
	if ( !str_cmp( name, "do_makepurpleeye"))       return do_makepurpleeye;
	if ( !str_cmp( name, "do_makeblueeye"))         return do_makeblueeye;
	if ( !str_cmp( name, "do_makegreeneye"))        return do_makegreeneye;
	if ( !str_cmp( name, "do_makeapplicator"))      return do_makeapplicator;
	if ( !str_cmp( name, "do_makerawdrug"))         return do_makerawdrug;
	...
	if ( !str_cmp( name, "do_mortality" ))		return do_mortality;
	...
	if ( !str_cmp( name, "do_mpsleep" ))		return do_mpsleep;
	...
	if ( !str_cmp( name, "do_newpassword"))		return do_newpassword;
	...
	if ( !str_cmp( name, "do_newclans" ))           return do_newclans;
	if ( !str_cmp( name, "do_new_silence"))		return do_new_silence;
	if ( !str_cmp( name, "do_new_unsilence"))	return do_new_unsilence;
	if ( !str_cmp( name, "do_newwhois"))		return do_newwhois;
	if ( !str_cmp( name, "do_newstudy"))		return do_newstudy;
	if ( !str_cmp( name, "do_newendurance"))	return do_newendurance;
	if ( !str_cmp( name, "do_newstretch"))		return do_newstretch;
	if ( !str_cmp( name, "do_newresearch"))		return do_newresearch;
	if ( !str_cmp( name, "do_newsocialize"))	return do_newsocialize;
	if ( !str_cmp( name, "do_newweapon" ))		return do_newweapon;
	break;
    case 'o':
...
/*	if ( !str_cmp( name, "do_opentourney" ))	return do_opentourney; */
	...
	if ( !str_cmp( name, "do_plead"))		return do_plead;
	break;
    case 'q':
	...
	if ( !str_cmp( name, "do_recover" ))		return do_recover;
	...
	if ( !str_cmp( name, "do_resettrade" ))		return do_resettrade;
	...
	if ( !str_cmp( name, "do_rumors" ))		return do_rumors;
	if ( !str_cmp( name, "do_rp") )                 return do_rp;
	if ( !str_cmp( name, "do_rpauthorize"))         return do_rpauthorize;
	if ( !str_cmp( name, "do_research"))		return do_research;
	if ( !str_cmp( name, "do_resuscitate"))		return do_resuscitate;
	break;
    case 's':
	...
	if ( !str_cmp( name, "do_sellgood" ))          return do_sellgood;
	...
	if ( !str_cmp( name, "do_setlevels" ))		return do_setlevels;
	...
	if ( !str_cmp( name, "do_setrestore" ))		return do_setrestore;
	if ( !str_cmp( name, "do_setslay" ))		return do_setslay;
	if ( !str_cmp( name, "do_setweapon" ))		return do_setweapon;
	...
	if ( !str_cmp( name, "do_spendqp" ))		return do_spendqp;
	...
	if ( !str_cmp( name, "do_surrender"))		return do_surrender;
	if ( !str_cmp( name, "do_systemtalk" ))		return do_systemtalk;
	if ( !str_cmp( name, "do_shadowbox"))		return do_shadowbox;
	if ( !str_cmp( name, "do_self_medicate"))	return do_self_medicate;
	if ( !str_cmp( name, "do_surgery"))		return do_surgery;
	if ( !str_cmp( name, "do_setclantalk"))		return do_setclantalk;
	...
	if ( !str_cmp( name, "do_top20"))		return do_top20;
	...
	if ( !str_cmp( name, "do_updatetrade" ))			return do_updatetrade;
	...
	if ( !str_cmp( name, "do_webwho"))		return do_webwho;
...
{
	if ( skill == do_empathize )	return "do_empathize";
	if ( skill == do_atrophy )     return "do_atrophy";
	if ( skill == do_newweapon )     return "do_newweapon";
	if ( skill == do_destroyweapon )     return "do_destroyweapon";
	if ( skill == do_showweapon )     return "do_showweapon";
	if ( skill == do_weapons )     return "do_weapons";
	if ( skill == do_setweapon )     return "do_setweapon";
	if ( skill == do_installcomponent )     return "do_installcomponent";
	if ( skill == do_detachcomponent )     return "do_detachcomponent";
    ...
    if ( skill == do_gather_intelligence )     return "do_gather_intellingence";
    if ( skill == do_add_patrol )     return "do_add_patrol";
    if ( skill == do_appeal )		return "do_appeal";
    if ( skill == do_addradio)		return "do_addradio";
    if ( skill == do_calm)		return "do_calm";
    if ( skill == do_attend )		return "do_attend";
    if ( skill == do_gainage )		return "do_gainage";
    if ( skill == do_special_forces )     return "do_special_forces";
    if ( skill == do_support )		return "do_support";
    if ( skill == do_setclantalk )	return "do_setclantalk";
    if ( skill == do_treat )		return "do_treat";
    if ( skill == do_surgery )		return "do_surgery";
    if ( skill == do_group_heal)	return "do_group_heal";
    if ( skill == do_self_medicate )	return "do_self_medicate";
    if ( skill == do_shadowbox )	return "do_shadowbox";
    if ( skill == do_applylicense )	return "do_applylicense";
    if ( skill == do_jail )     return "do_jail";
    if ( skill == do_junk )	return "do_junk";
    if ( skill == do_elite_guard )     return "do_elite_guard";
    if ( skill == do_endurance )       return "do_endurance";
    if ( skill == do_smalltalk )     return "do_smalltalk";
    if ( skill == do_propaganda )     return "do_propaganda";
    if ( skill == do_plead )		return "do_plead";
    if ( skill == do_pcrename )		return "do_pcrename";
    if ( skill == do_bandage )		return "do_bandage";
    if ( skill == do_bribe )     return "do_bribe";
    if ( skill == do_bootup )    return "do_bootup";
    if ( skill == do_mass_propaganda )     return "do_mass_propaganda";
    if ( skill == do_background ) return "do_background";
    ...
    if ( skill == do_surrender)	 return "do_surrender";
    if ( skill == do_train )     return "do_train";
    if ( skill == do_clone )     return "do_clone";
    if ( skill == do_commandeer) return "do_commandeer";
    ...
    if ( skill == do_makelaptop )	return "do_makelaptop";
    if ( skill == do_spacetalk )     return "do_spacetalk";
    if ( skill == do_shiphail )		return "do_shiphail";
...
//    if ( skill == do_hack_systems )     return "do_hack_systems";
    if ( skill == do_hackclanaccount )	return "do_hackclanaccount";
    if ( skill == do_hackcamera)	return "do_hackcamera";
    if ( skill == do_hackbank )		return "do_hackbank";
    if ( skill == do_hackbaydoor )     return "do_hackbaydoor";
    if ( skill == do_hackwho )		return "do_hackwho";
    if ( skill == do_hackbanktransfer)	return "do_hackbanktransfer";
    if ( skill == do_hackshipdoor)	return "do_hackshipdoor";
    if ( skill == do_hacklaunch)	return "do_hacklaunch";
    if ( skill == do_hackbroadcast)	return "do_hackbroadcast";
    if ( skill == do_hackbounty)	return "do_hackbounty";
    if ( skill == do_hail )     return "do_hail";
    if ( skill == do_appoint )     return "do_appoint";
    if ( skill == do_deceive )	return "do_deceive";
    if ( skill == do_demote )     return "do_demote";
    if ( skill == do_empower )     return "do_empower";
    if ( skill == do_capture )     return "do_capture";
    if ( skill == do_newsocialize) return "do_newsocialize";
    if ( skill == do_newresearch ) return "do_newresearch";
    if ( skill == do_newstretch )  return "do_newstretch";
    if ( skill == do_newstudy )	   return "do_newstudy";
    if ( skill == do_newendurance) return "do_newendurance";
    if ( skill == do_newclan )     return "do_newclan";
    if ( skill == do_newpassword)  return "do_newpassword";
    if ( skill == do_newwhois)	  return "do_newwhois";
    if ( skill == do_newclans)     return "do_newclans";
    if ( skill == do_new_silence)  return "do_new_silence";
    if ( skill == do_new_unsilence) return "do_new_unsilence";
    ...
    if ( skill == do_bankfreeze ) return "do_bankfreeze";
    ...
    if ( skill == do_resuscitate )	return "do_resuscitate";
    ...
    if ( skill == do_sellgood )		return "do_sellgood";
    ...
    if ( skill == do_afkmessage )	return "do_afkmessage";
    ...
    if ( skill == do_bigshot )		return "do_bigshot";
    ...
    if ( skill == do_buygood )	return "do_buygood";
    ...
    if ( skill == do_destroyrestore )	return "do_destroyrestore";
...
/*  if ( skill == do_diagnose )		return "do_diagnose";*/
    ...
    if ( skill == do_frenzy )		return "do_frenzy";
    if ( skill == do_get )		return "do_get";
    if ( skill == do_giveplanet )       return "do_giveplanet";
    ...
    if ( skill == do_lagout )		return "do_lagout";
    if ( skill == do_lobby )		return "do_lobby";
    ...
    if ( skill == do_lift )             return "do_lift";
    ...
    if ( skill == do_makerestore )	return "do_makerestore";
    if ( skill == do_makeslay )		return "do_makeslay";
    if ( skill == do_makeknife )	return "do_makeknife";
    if ( skill == do_makegun )   	return "do_makegun";
    if ( skill == do_makerifle )	return "do_makerifle";
    if ( skill == do_makesword )	return "do_makesword";
    if ( skill == do_makespice )	return "do_makespice";
    if ( skill == do_makebloodyeye )    return "do_makebloodyeye";
    if ( skill == do_makeyelloweye )    return "do_makeyelloweye";
    if ( skill == do_makepurpleeye )    return "do_makepurpleeye";
    if ( skill == do_makeblueeye )      return "do_makeblueeye";
    if ( skill == do_makegreeneye )     return "do_makegreeneye";
    if ( skill == do_makeapplicator )   return "do_makeapplicator";
    if ( skill == do_makerawdrug )      return "do_makerawdrug";
    if ( skill == do_none )             return "do_none";
    ...
    if ( skill == do_mortality )		return "do_mortality";
    if ( skill == do_mount )		return "do_mount";
    if ( skill == do_mpwarkill )	return "do_mpwarkill";
    ...
    if ( skill == do_mpsleep )		return "do_mpsleep";
...
/*  if ( skill == do_opentourney )	return "do_opentourney"; */
    ...
    if ( skill == do_recover )		return "do_recover";
    ...
    if ( skill == do_resettrade )	return "do_resettrade";
    ...
    if ( skill == do_rumors )		return "do_rumors";
    if ( skill == do_rp )               return "do_rp";
    if ( skill == do_rpauthorize )      return "do_rpauthorize";
    ...
    if ( skill == do_setlevels )	return "do_setlevels";
    if ( skill == do_setship )          return "do_setship";
    if ( skill == do_setrestore )	return "do_setrestore";
    ...
    if ( skill == do_showrestore )	return "do_showrestore";
    ...
    if ( skill == do_shutoff )		return "do_shutoff";
	...
	if ( skill == do_top20 )		return "do_top20";
    ...
    if ( skill == do_updatetrade )		return "do_updatetrade";
    ...
    if ( skill == do_webwho )		return "do_webwho";
