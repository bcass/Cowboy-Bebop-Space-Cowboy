...
void do_fakequit( CHAR_DATA *ch, char *argument )
{
    char buf[MAX_STRING_LENGTH];
  ...
  if ( arg2[0]=='\0' || !str_cmp( arg2,"accept" ) || !str_cmp( arg2,"yes" ))
	...
	ch_printf( ch, "You have authorized %s.\n\r", victim->name);
     ...
     /* Below sends a message to player when name is accepted - Brittany   */
        ...
        "You are now fully authorized to play Cowboy Bebop:Space Cowboy.\n\r",victim->name);
     return;
     }
...
void do_rpauthorize( CHAR_DATA *ch, char *argument )
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char buf[MAX_STRING_LENGTH];
  ...
  if ( arg1[0] == '\0' )
     {
     send_to_char( "Usage:  rpauthorize <player>\n\r", ch );
     return;
    }
  ...
  if( victim == NULL )
  {
	send_to_char("Player is not present.\n\r", ch);
	return;
  }

  if( IS_NPC(victim))
  {
	send_to_char("Why would they need that?\n\r", ch);
	return;
  }

  if ( arg2[0]=='\0' || !str_cmp( arg2,"yes" ))
     {
        if(!IS_SET(victim->pcdata->flags, PCFLAG_RPAUTH))
        {
        SET_BIT(victim->pcdata->flags, PCFLAG_RPAUTH);
	ch_printf(ch,"You have authorized %s to RP.\n\r",victim->name);
	ch_printf( victim,
	"You have been authorized to RP!\n\r");
	}
	else
	{
	REMOVE_BIT(victim->pcdata->flags, PCFLAG_RPAUTH);
	ch_printf(ch, "You removed %s from being able to RP!", victim->name);
        ch_printf( victim,
        "You got your RP ability removed!\n\r");
	}
	return;
     }
    /*
    if ( !str_cmp( arg2, "no") )
     {
	if(IS_SET(ch->pcdata->flags, PCFLAG_RPAUTH) )
        {
	REMOVE_BIT(victim->pcdata->flags, PCFLAG_RPAUTH);
	ch_printf(ch, "You removed %s from being able to RP!", victim->name);
	ch_printf( victim,
	...
	{
	send_to_char("They aren't authorized to RP anyways!\n\r", ch);
	}
	return;
     }
    */
...
{
 char add_len[MAX_INPUT_LENGTH]; /* Following defines are for putting spaces into the rank */
  int sp1, sx;                    /* To fix the color problem. */
  char finrank[MAX_INPUT_LENGTH];
  ...
  sp1 = strlen_color(argument);   /* Finds the strlen_color of the argument */
  sprintf( add_len, " ");         /* Init. the add_len strength */

      for (sx=19; sx > sp1; sx--) /* 15 - 1 for add_len init.*/
            strcat( add_len, " "); /* For every time it will add a blank space after the rank */
  ...
  if((strlen(argument) > 40) || strlen_color(argument) > 19)
  {
	send_to_char("&RRank is too long, choose a smaller one.\n\r", ch);
	return;
  }
  ...
  if ( !str_cmp( argument, "none" ) )
    ...
    ch->pcdata->rank = str_dup( finrank ); /* If there is a rank, it will use finrank instead of */
...
/*  Wow, this is annoying as hell! Let's remove it - Gatz
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    ch_printf( ch, "Vnum: %d   Sex: %s   Room: %d   Count: %d  Killed: %d  Home: %d\n\r",
	...
	IS_NPC(victim) ? 0 : victim->plr_home ? victim->plr_home->vnum : 0
	);
	...
	get_curr_lck(victim),
	get_curr_frc(victim) );
    ch_printf( ch, "Hps: %d/%d  Force: %d/%d   Move: %d/%d   PC Age: %d\n\r",
...
ch_printf( ch,
	"Top Level: %d     Race: %d  Align: %d  AC: %d  Wulongs: %lld\n\r",
    ...
    if ( !IS_NPC(victim) )
	ch_printf( ch,
	    "Thirst: %d   Full: %d   Drunk: %d     QP: %d/%d     Bl: %d  Bank: %lld\n\r",
    ...
    {
            ch_printf( ch, "MentalState: %d   EmotionalState: %d    Jail Time: %d\n\r",
                        victim->mental_state, victim->emotional_state, victim->pcdata->bountyrelease );
    }
    ...
    ch_printf( ch, "Hours: %d   Seconds Played: %d   Timer: %d   Act: %d\n\r",
    ...
    if(!IS_NPC(victim))
    {
	ch_printf( ch, "Str Training: %d\n\r", victim->strtrain);
	ch_printf( ch, "Wis Training: %d\n\r", victim->wistrain);
	ch_printf( ch, "Int Training: %d\n\r", victim->inttrain);
	ch_printf( ch, "Con Training: %d\n\r", victim->contrain);
	ch_printf( ch, "Dex Training: %d\n\r", victim->dextrain);
	ch_printf( ch, "Cha Training: %d\n\r", victim->chatrain);
	ch_printf( ch, "Atrophying: %d\n\r", victim->pcdata->atrophy);
    }
    return;
}
...
/*
void do_setmax( CHAR_DATA *ch, char *argument )
{
    char arg[MAX_INPUT_LENGTH];
    char buf[MAX_STRING_LENGTH];
    int number;
   ...
   strcpy( arg, argument );
    ...
    if(number < 1)
    {
	send_to_char("Number must be larger than 1!\n\r", ch);
	return;
    }
...
void do_reboot( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "Please remain seated. Reboot by &C%s&w", ch->name );
    do_echo( ch, buf );
...
void do_shutdown( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "Please come back later. Shutdown by &C%s&w", ch->name );
...
void do_qpstat( CHAR_DATA *ch, char *argument )
{
  ...
  if ( ( victim = get_char_world( ch, arg ) ) == NULL )
  {
    send_to_char( "No one by that name currently in the galaxy.\n\r", ch );
    return;
  }
  if ( IS_NPC( victim ) )
  {
    send_to_char( "Mobs don't have quest points.\n\r", ch );
    return;
  }
  ch_printf( ch, "%s has %d quest points.\n\r",
        victim->name,
        victim->pcdata->quest_curr );
  return;
}
...
*/
    ...
    {
      ch_printf( ch, "Unknown error #%d - %s (immortal data).  Report to Jet or Spike\n\r",
    ...
    for ( pArea = first_build; pArea; pArea = pArea->next )
      if ( !strcmp( pArea->filename, buf2 ) )
      {
        ...
        {
          ch_printf( ch, "Unknown error #%d - %s (area data).  Report to Jet or Spike.\n\r",
                  errno, strerror( errno ) );
...
void do_immortalize( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char("You can only immortalize up to your level",ch);
		return;
	}
...
void do_freeze( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "Don't freeze other immortals!\n\r", ch );
	return;
    }
...
void do_new_silence( CHAR_DATA *ch, char *argument )
{
  CHAR_DATA *victim;
  char arg[MAX_INPUT_LENGTH];
  sh_int time;
  ...
  if ( !*arg )
  {
    send_to_char( "Silence whom, and for how long?\n\r", ch );
    return;
  }
  ...
  if ( IS_IMMORTAL(victim) )
  {
    send_to_char( "There is no point in silenceing an immortal.\n\r", ch );
    return;
  }
  if ( victim->pcdata->unsilence_date != 0 )
  {
    ch_printf(ch, "They are already in silence until %24.24s, by %s.\n\r",
            ctime(&victim->pcdata->unsilence_date), victim->pcdata->silenceed_by);
    return;
  }
  argument = one_argument(argument, arg);
  if ( !*arg || !is_number(arg) )
  {
    send_to_char( "Silence them for how long?\n\r", ch );
    return;
  }
  time = atoi(arg);
  if ( time <= 0 )
  ...
  victim->pcdata->silenceed_by = STRALLOC(ch->name);
  ch_printf(ch, "%s will be released from silence at %24.24s.\n\r", victim->name,
          ctime(&victim->pcdata->unsilence_date));
  act(AT_MAGIC, "$n disappears in a cloud of silenceish light.", victim, NULL, ch, TO_NOTVICT);
  SET_BIT(victim->act, PLR_SILENCE);
  act(AT_MAGIC, "$n appears in a could of silenceish light.", victim, NULL, ch, TO_NOTVICT);
  ch_printf(victim, "The immortals are not pleased with your actions.\n\r"
          "You shall remain in silence for %d %s%s.\n\r", time,
...
void do_new_unsilence( CHAR_DATA *ch, char *argument )
{
  CHAR_DATA *victim;
  char arg[MAX_INPUT_LENGTH];
  ...
  if ( IS_NPC(victim) || !IS_SET(victim->act, PLR_SILENCE) || victim->pcdata->unsilence_date == 0 )
  {
    send_to_char( "No one like that is in silence.\n\r", ch );
    return;
  }


  send_to_char( "The gods have smiled on you and released you from silence early!\n\r", victim );
  send_to_char( "They have been released.\n\r", ch );

  /* Crashy, crashy!
  if ( victim->pcdata->silenceed_by )
  {
    if( str_cmp(ch->name, victim->pcdata->silenceed_by) )
      ch_printf(ch, "(You should probably write a note to %s, explaining the early release.)\n\r",
            victim->pcdata->silenceed_by);
    STRFREE(victim->pcdata->silenceed_by);
    victim->pcdata->silenceed_by = NULL;
   ...
   if (victim->plr_home != NULL)
     {
      ...
      SET_BIT( room->room_flags , ROOM_EMPTY_HOME );
        sprintf( buf3, "%s%c/%s.home", PLAYER_DIR, tolower(victim->name[0]),
                                 capitalize( victim->name ) );
	remove(buf3);
      fold_area( room->area, room->area->filename, FALSE );
          victim->plr_home = NULL;
     }
    //Players lose bounties
...
void do_cset( CHAR_DATA *ch, char *argument )
{
  ...
  if (argument[0] == '\0')
  {
    ...
    ch_printf(ch, "Automatic Big Shot Statis: %s",
	    sysdata.big_shot ? "Off" : "On" );
  ...
  if( !str_cmp( arg, "big shot") )
  {
    sysdata.big_shot = !sysdata.big_shot;
    if( sysdata.big_shot )
	send_to_char("Auto-Big Shot is now off\n\r", ch);
    else
	send_to_char("Auto-Big Shot is now on\n\r", ch);
    return;
  }
...
void do_sedit( CHAR_DATA *ch, char *argument )
{
    ...
    /* Removed get_trust(ch) > LEVEL_LESSER because Imms who have
	this skill should be able to save! - Gatz */
...
void do_cedit( CHAR_DATA *ch, char *argument )
{
	...
	ch_printf( ch, "Command:  %s\n\rLevel:    %d\n\rPosition: %d\n\rLog:      %d\n\rCode:     %s\n\rOOC:      %d\n\r",
	    ...
	    skill_name(command->do_fun), command->ooc );
...
/*
 * IPcompare by Shaddai - imported into SWR by Kristen
 */
...
void do_mortality( CHAR_DATA *ch, char *argument )
{
    ...
    argument = one_argument( argument, arg );
    if ( arg[0] == '\0' && (!IS_IMMORTAL(ch) && ch->tmplevel < 101))
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
    if (IS_IMMORTAL(ch))
    {
     ...
     {
	send_to_char( "Invalid Mortal Level.\n\r", ch );
        return;
     }

     ch->tmplevel = ch->top_level;
    ...
    if (!IS_IMMORTAL(ch) && ch->tmplevel > 100)
    {
    ch->top_level = ch->tmplevel;
 ...
 */
// So buggy, had to fix it up - Gatz
...
void do_pcrename( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( newname, "%s%c/%s", PLAYER_DIR, tolower(arg2[0]),
                                 capitalize( arg2 ) );
    sprintf( oldname, "%s%c/%s",PLAYER_DIR,tolower(victim->name[0]),
                                 capitalize( victim->name ) );
    sprintf( backname,"%s%c/%s",BACKUP_DIR,tolower(victim->name[0]),
                                 capitalize(victim->name ) );
...
int fixitem(CHAR_DATA *victim, OBJ_DATA *eq) /* Added for more modular design of fixac (see below) - Funf */
{
	if(!eq || !victim) return;

	if(eq->item_type == ITEM_ARMOR)
	{
		if(eq->value[1] >= 15 && eq->wear_loc != WEAR_SHIELD)
		{
			bug("%s is wearing %s with %d ac, correcting", victim->name, eq->name, eq->value[1]);
			eq->value[0] = 12;
			eq->value[1] = 12;
		} else if (eq->value[1] > 20 && eq->wear_loc == WEAR_SHIELD)
		{
			bug("%s is wearing %s with %d ac, correcting", victim->name, eq->name, eq->value[1]);
			eq->value[0] = 20;
			eq->value[1] = 20;
		...
		{
			/*ac -= eq->value[0];*/
			return apply_ac(eq, eq->wear_loc); /* Fix ac bug - 2009-11-11 Funf */
		}
	}

	if(eq->item_type == ITEM_WEAPON)
	{
		if( (eq->pIndexData->vnum == 1800 || eq->pIndexData->vnum == 2234 || eq->pIndexData->vnum == 3665) && eq->value[1] > 3)
		{
			bug("%s is wearing %s with %d mindam, correcting", victim->name, eq->name, eq->value[1]);
			eq->value[1] = 3;
		}
		else if(eq->value[1] > 8)
		{
			bug("%s is wearing %s with %d mindam, correcting", victim->name, eq->name, eq->value[1]);
			eq->value[1] = 8;
		}

		if( (eq->pIndexData->vnum == 1800 || eq->pIndexData->vnum == 2234 || eq->pIndexData->vnum == 3665) && eq->value[2] > 13)
		{
			bug("%s is wearing %s with %d maxdam, correcting", victim->name, eq->name, eq->value[2]);
			eq->value[2] = 13;
		}
		else if(eq->value[2] > 27)
		{
			bug("%s is wearing %s with %d maxdam, correcting", victim->name, eq->name, eq->value[2]);
			eq->value[2] = 27;
		}
	...
	if(eq->item_type == ITEM_CONTAINER)
	{
		OBJ_DATA *tempobj;
		for(tempobj = eq->first_content;tempobj;tempobj = tempobj->next_content)
			fixitem(victim, tempobj);
	}
	return 0;
}

void do_fixac(CHAR_DATA *ch, char *argument) /* I messed up some people's ac, so... - Funf */
{
	CHAR_DATA *victim;
	OBJ_DATA *eq;
	char arg1[MAX_INPUT_LENGTH];
	...
	for(eq = victim->first_carrying;eq;eq = eq->next_content)
		ac -= fixitem(victim, eq);

	if(victim->pcdata && victim->plr_home)
	{
		for(eq = victim->plr_home->first_content;eq;eq = eq->next_content)
			fixitem(victim, eq);
	}
